	...
	plugins: [
		...
		filesize({
			...
			reporter: (
				options,
				bundle,
				{ fileName, bundleSize, minSize, gzipSize },
			) => {
				// Only show warnings for JS files (not chunks) that are large
				if (bundleSize > 500000 && !fileName.includes("chunks/")) {
					console.warn(
						`\x1b[33mWarning: ${fileName} is large (${Math.round(bundleSize / 1024)}KB)\x1b[0m`,
					);
				}
				...
				if (buildDetail && bundleSize && !isNaN(bundleSize)) {
					...
					if (originalKB > 0) {
						console.log(
							`\x1b[36m${fileName}:\x1b[0m ${originalKB}KB → ${gzippedKB}KB gzipped`,
						);
					}
				}
			},
