...
const onwarn = (warning, warn) => {
  ...
  if (warning.code === 'DEPRECATED_FEATURE') {
    console.warn(`\x1b[33mDeprecation warning:\x1b[0m ${warning.message}`);
  }
  ...
  if (buildDetail || warning.code === 'CIRCULAR_DEPENDENCY') {
    ...
    console.warn(`\x1b[33mWarning:\x1b[0m [${warning.code}] at ${location}`);
    console.warn(`  ${warning.message}`);
    
    // Show additional details for circular dependencies
    if (warning.code === 'CIRCULAR_DEPENDENCY') {
      console.warn(`  Cycle: ${warning.cycle.join(' -> ')}`);
    }
  ...
  plugins: [
    ...
    postcss({
      ...
      plugins: [
        ...
        production && cssnano({ // Minify CSS in production
          preset: 'default',
        })
      ],
      sourceMap: !production,
    ...
    filesize({
      ...
      reporter: (options, bundle, { fileName, bundleSize, minSize, gzipSize }) => {
        ...
        if (bundleSize > 500000 && !fileName.includes('chunks/')) {
          console.warn(`\x1b[33mWarning: ${fileName} is large (${Math.round(bundleSize / 1024)}KB)\x1b[0m`);
        }
        
        // Show more detailed information when BUILD_DETAIL is true
        if (buildDetail && bundleSize && !isNaN(bundleSize)) {
          const originalKB = Math.round(bundleSize / 1024);
          const minifiedKB = Math.round((minSize || 0) / 1024);
          const gzippedKB = Math.round((gzipSize || 0) / 1024);
          
          if (originalKB > 0) {
            console.log(`\x1b[36m${fileName}:\x1b[0m ${originalKB}KB → ${gzippedKB}KB gzipped`);
          }
        }
      }
    }),
