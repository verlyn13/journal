 ...
 */
function createWrappingTransaction(
	state,
	prefix,
	suffix,
	placeholder,
	...
	if (range.empty) {
		...
		return {
			...
			), // Keep selection, adjusted for prefix
		};
	}
}
...
export function insertList(view, type) {
	...
	if (spec.changes.length > 0) {
		view.dispatch(
			view.state.update(spec, { userEvent: `input.format.${type}` }),
		);
	}
	view.focus();
}
...
export function insertBlockquote(view) {
	...
	if (spec.changes.length > 0) {
		view.dispatch(
			view.state.update(spec, { userEvent: "input.format.blockquote" }),
		);
	}
	view.focus();
}
...
export function insertMarkdownSyntax(
	...
	if (block) {
		...
		if (
			startLine.from > 0 &&
			state.doc.sliceString(startLine.from - 1, startLine.from) !== "\n"
		) {
			insertPrefix = "\n" + insertPrefix;
		}
		if (startLine.number === endLine.number && startLine.length === 0) {
			...
			if (
				...
				state.doc.sliceString(endLine.to, endLine.to + 1) !== "\n"
			) {
				insertSuffix = "\n" + insertSuffix;
		...
		if (range.empty) {
			...
			changes.push({
				from: range.from,
				insert: insertPrefix + placeholder + insertSuffix,
			});
		} else {
			// Wrap selection
			...
			range.empty && !block
				...
				: range.empty && block
					? EditorSelection.cursor(
							startLine.from + insertPrefix.length + 1 + placeholder.length + 1,
						) // Place cursor inside block placeholder
