 ...
 * </div>
 */
export default function editorComponent(entryId = null) { // Removed initialContent parameter
    return {
        // --- State ---
         ...
         */
        init() {
            console.log(`[EditorComponent] Initializing for entryId: ${this.entryId || 'new'}`);
            ...
            if (scriptTag) {
                try {
                    ...
                    console.log(`[EditorComponent] Parsed initial content from #${scriptTagId}. Length: ${initialContent.length}`);
                } catch (e) {
                    console.error(`[EditorComponent] Error parsing initial content JSON from #${scriptTagId}:`, e);
                    initialContent = ''; // Fallback to empty string on error
                }
            } else {
                console.warn(`[EditorComponent] Initial content script tag #${scriptTagId} not found. Editor will start empty.`);
            }
            ...
            if (successFlash) {
                console.log('[EditorComponent] Success flash message detected, clearing draft.');
            ...
            const observer = new MutationObserver((mutationsList, obs) => {
                ...
                const editorTargetElement = this.$el.querySelector('[x-ref="editorElement"]');

                if (editorTargetElement) {
                    console.log('[EditorComponent/Observer] Found editorElement, initializing CodeMirror...');
                    obs.disconnect(); // Stop observing once found

                    // Log the content before creating the editor
                    console.log('[EditorComponent/Observer] Content being passed to createEditor:', this.content);
                    ...
                    try {
                        ...
                        console.log('[EditorComponent/Observer] CodeMirror editor created successfully.');

                        // Initial preview update if starting in split/preview mode
                        if (this.mode === 'split' || this.mode === 'preview') {
                            console.log(`[EditorComponent/Observer] Initial mode is '${this.mode}', triggering initial preview update.`);
                            this.updatePreview();
                        }

                    } catch (error) {
                         console.error('[EditorComponent/Observer] Error creating CodeMirror editor:', error);
                    }
            ...
            this.$nextTick(() => {
                 console.log('[EditorComponent/init] Starting MutationObserver to find editorElement.');
                 // Check if element already exists before observing (edge case)
                 const initialCheckElement = this.$el.querySelector('[x-ref="editorElement"]');
                 if (initialCheckElement) {
                     console.log('[EditorComponent/init] editorElement found immediately, initializing without observer.');
                     // Directly initialize if found immediately (avoids observer callback complexity)
                     try {
                         // Log the content before creating the editor
                         console.log('[EditorComponent/init] Content being passed to createEditor (immediate check):', this.content);
                         this.editorView = createEditor(
                             initialCheckElement, this.content, { onChange: (newContent) => { this.content = newContent; this.handleContentChange(); } }
                         );
                         console.log('[EditorComponent/init] CodeMirror editor created successfully (immediate check).');
                         if (this.mode === 'split' || this.mode === 'preview') { this.updatePreview(); }
                     } catch (error) {
                          console.error('[EditorComponent/init] Error creating CodeMirror editor (immediate check):', error);
                     }
                 } else {
            ...
            window.addEventListener('beforeunload', () => {
                console.log('[EditorComponent/beforeunload] Saving draft before page unload...');
                this.saveDraftIfNeeded();
            });
        ...
        handleContentChange: debounce(function() {
            ...
            console.log('[EditorComponent/handleContentChange] Content changed (debounced).');
        ...
        setMode(newMode) {
            if (['edit', 'split', 'preview'].includes(newMode)) {
                ...
                if ((newMode === 'split' || newMode === 'preview') && !this.previewHtml) {
                    // Fetch preview immediately if switching to a mode that needs it
                    console.log(`[EditorComponent/setMode] Mode requires preview ('${newMode}'), triggering update.`);
                    this.updatePreview();
                }
        ...
        async updatePreview() {
            ...
            if (!csrfToken) {
                console.error('[EditorComponent/updatePreview] CSRF token meta tag not found!');
                this.previewHtml = '<p class="error">Configuration error: CSRF token missing.</p>';
                this.isLoadingPreview = false;
                return;
            }

            try {
                const response = await fetch('/api/v1/markdown/preview', { // Corrected URL
                    method: 'POST',
                    headers: {
                ...
                if (!response.ok) {
                    ...
                    console.error(`[EditorComponent/updatePreview] Preview API request failed: ${response.status}`, errorText);
                ...
                this.$nextTick(() => {
                    ...
                    if (previewElement && window.MathJax && window.MathJax.typesetPromise) {
                        ...
                        window.MathJax.typesetPromise([previewElement]).catch((err) => console.error('[EditorComponent/updatePreview/$nextTick] MathJax typesetting failed:', err));
                    } else if (!previewElement) {
                         console.warn("[EditorComponent/updatePreview/$nextTick] Preview element (x-ref='previewContent') not found for MathJax.");
                    } else {
                        console.warn('[EditorComponent/updatePreview/$nextTick] MathJax not available or not configured for typesetting.');
                    }
                });

            } catch (error) {
                console.error('[EditorComponent/updatePreview] Error fetching Markdown preview:', error);
        ...
        saveDraftIfNeeded() {
            ...
            if (this.content !== this.lastSavedContent) {
                console.log('[EditorComponent/saveDraftIfNeeded] Content changed, saving draft...');
         ...
         */
        clearDraftOnSubmit() {
            console.log('[EditorComponent/clearDraftOnSubmit] Form submitted, clearing draft.');
            if (this.persistence) {
                this.persistence.clearDraft();
            } else {
                console.warn('[EditorComponent/clearDraftOnSubmit] Persistence object not available.');
            }
        },
        ...
        insertMarkdown(type) {
            if (!this.editorView) {
                console.warn('[EditorComponent/insertMarkdown] EditorView not available.');
                return;
            }
            ...
            switch (type) {
                ...
                case 'table':
                     ...
                     break;
                case 'codeblock':
                     insertMarkdownSyntax(this.editorView, '```\n', '\n```', 'code here', true);
                     break;
                default:
                    console.warn(`[EditorComponent/insertMarkdown] Unknown markdown type: ${type}`);
            }
        },
