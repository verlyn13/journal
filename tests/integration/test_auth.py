...
def test_successful_registration(test_client, test_app):
    ...
    assert b"Congratulations, you are now a registered user!" in response.data # Check flash message
...
def test_registration_duplicate_username(test_client, test_app):
    ...
    with test_app.app_context(): # Add app context
        response = test_client.post(url_for('auth.register'), data={
            ...
            'password': 'password123',
            'confirm_password': 'password123'
        }, follow_redirects=True) # Can follow redirects
...
def test_registration_duplicate_email(test_client, test_app):
    ...
    with test_app.app_context(): # Add app context
        response = test_client.post(url_for('auth.register'), data={
            ...
            'password': 'password123',
            'confirm_password': 'password123'
        }, follow_redirects=True) # Can follow redirects now

    # Form validation fails, so it re-renders the registration page (200 OK)
    assert response.status_code == 200
    # Check for the validation error message from the form's validate_email method
    assert b"Email address already registered. Please use a different one or log in." in response.data
    assert b"Register" in response.data # Still on register page
...
def test_successful_login_logout(test_client, test_app):
    ...
    # --- Test Login ---
    with test_app.app_context(): # Add app context
        response_login = test_client.post(url_for('auth.login'), data={
            'username': username,
            'password': password
    ...
    with test_app.app_context(): # Add app context
        ...
        response_protected = test_client.get(url_for('main.index'), follow_redirects=False) # Don't follow redirect
    assert response_protected.status_code == 302 # Redirect status
    # Need app context to generate comparison URL
    with test_app.app_context():
        login_url = url_for('auth.login')
    assert login_url.split('://', 1)[1].split('/', 1)[1] in response_protected.location # Check path part of URL
