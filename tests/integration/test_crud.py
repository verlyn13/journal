...
from journal.models import Entry, Tag, User
...
def create_test_entry(test_app, user_id, title="Initial Title", body="Initial Body"):
    with test_app.app_context():
        ...
        if not user:
            pytest.fail(
                f"User with id {user_id} not found in create_test_entry context."
            )
        ...
        committed_entry = (
            Entry.query.filter_by(user_id=user_id, title=title)
            .order_by(Entry.id.desc())
            .first()
        )
        if not committed_entry:
            pytest.fail(
                f"Could not find committed entry with title {title} for user {user_id}."
            )
...
def test_index_loads_authenticated(auth_client, test_app):  # Add test_app
    ...
    with test_app.app_context():
        ...
        assert f"Welcome, {user.username}!".encode() in response.data
...
def test_edit_entry_page_loads(auth_client, test_app):
    ...
    assert (
        b'value="edit, original"' in response.data
        or b'value="original, edit"' in response.data
    )
...
def test_update_entry(auth_client, test_app):
    ...
    updated_tags = (
        " keep, new_tag , UPDATE "  # Remove 'update', keep 'keep', add 'new_tag'
    )

    with test_app.app_context():  # Add app context
        response = client.post(
            url_for("main.edit_entry", entry_id=entry_id),
            data={"title": updated_title, "body": updated_body, "tags": updated_tags},
            follow_redirects=True,
        )
...
def test_delete_entry(auth_client, test_app):
    ...
    entry_id = create_test_entry(
        test_app, user_id, title="To Be Deleted", body="Delete Me"
    )  # Pass user_id
    ...
    with test_app.app_context():  # Add app context
        response = client.post(
            url_for("main.delete_entry", entry_id=entry_id), follow_redirects=True
        )
...
def test_access_non_owned_entry_forbidden(auth_client, test_app):
    ...
    client, _user1_id = auth_client  # This is user1's ID
    other_entry_id = None
    user2_id = None
    ...
    with test_app.app_context():
        ...
        other_entry_id = create_test_entry(
            test_app, user2_id, title="Other User Entry"
        )  # Pass user2_id
    ...
    with test_app.app_context():  # Add app context
        response_view = client.get(
            url_for("main.entry_detail", entry_id=other_entry_id)
        )
    assert response_view.status_code == 403

    with test_app.app_context():  # Add app context
        response_edit_get = client.get(
            url_for("main.edit_entry", entry_id=other_entry_id)
        )
    assert response_edit_get.status_code == 403

    with test_app.app_context():  # Add app context
        response_edit_post = client.post(
            url_for("main.edit_entry", entry_id=other_entry_id),
            data={"title": "hack", "body": "hack"},
        )
    assert response_edit_post.status_code == 403

    with test_app.app_context():  # Add app context
        response_delete = client.post(
            url_for("main.delete_entry", entry_id=other_entry_id)
        )
...
def test_access_nonexistent_entry_404(auth_client, test_app):  # Add test_app
    ...
    with test_app.app_context():  # Add app context
        response_view = client.get(
            url_for("main.entry_detail", entry_id=non_existent_id)
        )
    ...
    with test_app.app_context():  # Add app context
        response_delete = client.post(
            url_for("main.delete_entry", entry_id=non_existent_id)
        )
...
def test_filter_by_tag(auth_client, test_app):
    ...
    with test_app.app_context():
        ...
        if not tag_common_u2:
            pytest.fail(
                "Tag 'common' not found in the second context of test_filter_by_tag."
            )
    ...
    assert (
        b"Other User Common" not in response.data
    )  # Ensure other user's entry isn't shown
    ...
    assert (
        b"You haven't written any entries yet." in response.data
    )  # Or similar empty message
