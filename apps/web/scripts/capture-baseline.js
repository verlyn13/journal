...
try {
  ...
  const testOutput = execSync("bun run test:run --reporter=json", {
    encoding: "utf8",
    stdio: "pipe",
  })
  ...
  baseline.testResults = { summary: "Tests not run", error: _e2.message }
}
...
console.log(
  "- Total bundle size:",
  Object.values(baseline.bundleSize).reduce((a, b) => a + b, 0),
  "bytes",
)
