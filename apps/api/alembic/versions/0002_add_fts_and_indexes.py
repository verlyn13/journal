...
def upgrade() -> None:
    ...
    op.add_column('entries', sa.Column('search_vector', postgresql.TSVECTOR(),
                                      sa.Computed("to_tsvector('english', coalesce(title, '') || ' ' || coalesce(content, ''))",
                                                persisted=True), nullable=True))

    # Create GIN index for full-text search
    op.create_index('ix_entries_search_vector', 'entries', ['search_vector'],
