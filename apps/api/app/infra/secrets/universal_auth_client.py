...
class UniversalAuthClient:
    ...
    def __init__(
        ...
        self.server_url = server_url or os.getenv("INFISICAL_API_URL", "https://app.infisical.com/api")
    ...
    def from_env(cls, identity_prefix: str = "TOKEN_SERVICE") -> UniversalAuthClient:
        ...
        if not client_id:
            raise UniversalAuthError(f"UA_CLIENT_ID_{identity_prefix} environment variable required")
            
        client_secret = os.getenv(f"UA_CLIENT_SECRET_{identity_prefix}")
        if not client_secret:
            raise UniversalAuthError(f"UA_CLIENT_SECRET_{identity_prefix} environment variable required")
    ...
    async def _login(self) -> None:
        ...
        try:
            ...
            metrics_inc("infisical_auth_login_total", {"method": "universal_auth", "status": "success"})
            metrics_inc("infisical_auth_login_duration_seconds", {"method": "universal_auth"}, duration)
            
        except TimeoutError as e:
            logger.error("Universal Auth login timeout")
            metrics_inc("infisical_auth_login_total", {"method": "universal_auth", "status": "timeout"})
            raise UniversalAuthError("Login timeout") from e
        except Exception as e:
            logger.exception("Universal Auth login failed")
            metrics_inc("infisical_auth_login_total", {"method": "universal_auth", "status": "error"})
