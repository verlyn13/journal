...
class UserSession(Base):
    ...
    refresh_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), unique=True, index=True, default=uuid4)
    user_agent: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    ip_address: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    issued_at: Mapped[datetime] = mapped_column(default=datetime.utcnow, nullable=False, index=True)
    last_used_at: Mapped[datetime] = mapped_column(default=datetime.utcnow, nullable=False)
...
class ProcessedEvent(Base):
    ...
    processed_at: Mapped[datetime] = mapped_column(default=datetime.utcnow, index=True, nullable=False)
