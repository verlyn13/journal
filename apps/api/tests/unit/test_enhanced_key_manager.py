...
class TestInfisicalKeyManager:
    ...
    @pytest.mark.asyncio
    async def test_get_token_cipher_from_infisical(self, key_manager, mock_redis, mock_infisical_client):
    ...
    async def test_rotate_aes_keys_success(self, key_manager, mock_infisical_client):
        ...
        with patch.object(key_manager, "get_token_cipher", return_value=mock_cipher), \
             patch.object(key_manager, "_check_aes_rotation_needed", return_value=(True, "Test rotation")):
    ...
    async def test_rotate_aes_keys_not_needed(self, key_manager):
        ...
        with patch.object(key_manager, "_check_aes_rotation_needed", return_value=(False, "Not needed")):
    ...
    async def test_migrate_keys_to_infisical_success(self, key_manager):
        ...
        with patch.object(key_manager.__class__.__bases__[0], "get_current_signing_key", return_value=mock_jwt_key), \
             patch.object(TokenCipher, "from_env", return_value=mock_aes_cipher), \
             patch("app.infra.crypto.key_generation.Ed25519KeyGenerator.serialize_key_pair") as mock_serialize:
    ...
    async def test_migrate_keys_no_existing_keys(self, key_manager):
        ...
        with patch.object(key_manager.__class__.__bases__[0], "get_current_signing_key", side_effect=RuntimeError("No keys")), \
    ...
    async def test_health_check_all_healthy(self, key_manager, mock_redis, mock_infisical_client):
        ...
        with patch.object(key_manager.__class__.__bases__[0], "verify_key_integrity", return_value=jwt_health), \
    ...
    async def test_health_check_jwt_unhealthy(self, key_manager, mock_redis):
        ...
        with patch.object(key_manager.__class__.__bases__[0], "verify_key_integrity", side_effect=Exception("JWT error")):
