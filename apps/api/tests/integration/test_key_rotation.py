...
class TestKeyRotationScenarios:
    ...
    async def test_complete_rotation_cycle(self, key_manager, mock_redis, mock_infisical):
        ...
        with patch.object(key_manager, "_store_key_metadata", side_effect=store_metadata_side_effect), \
    ...
    async def test_overlap_window_verification(self, key_manager, mock_redis, mock_infisical):
        ...
        with patch.object(key_manager, "_get_current_key_metadata") as mock_current_meta, \
    ...
    @pytest.mark.asyncio
    async def test_key_rotation_with_cache_invalidation(self, key_manager, mock_redis, mock_infisical):
        ...
        with patch.object(key_manager, "_get_next_key") as mock_get_next, \
...
class TestRotationPerformance:
    ...
    async def test_rotation_completion_time(self, key_manager):
        ...
        with patch.object(key_manager, "_get_next_key") as mock_next, \
             patch.object(key_manager, "_promote_next_to_current"), \
             patch.object(key_manager, "_generate_next_key"), \
            ...
            assert rotation_time < 100, f"Rotation took {rotation_time:.2f}ms, should be under 100ms"
    ...
    async def test_verification_keys_retrieval_performance(self, key_manager):
        ...
        with patch.object(key_manager, "get_current_signing_key") as mock_current, \
            ...
            assert avg_time < 5, f"Average verification keys retrieval {avg_time:.2f}ms exceeds 5ms limit"
...
class TestRotationErrorHandling:
    ...
    async def test_rotation_with_infisical_timeout(self, key_manager, mock_infisical):
        ...
        async def timeout_fetch(*args, **kwargs):
            ...
            raise asyncio.TimeoutError("Infisical request timed out")
