...
async def test_outbox_retry_schedules_next_attempt(monkeypatch, db_session: AsyncSession):
  ...
  row = (await db_session.execute(text("SELECT attempts, next_attempt_at, state FROM events WHERE id=:id"), {"id": str(ev.id)})).first()
...
async def test_outbox_dead_letter_on_exhaustion(monkeypatch, db_session: AsyncSession):
  ...
  row = (await db_session.execute(text("SELECT attempts, state FROM events WHERE id=:id"), {"id": str(ev.id)})).first()
  assert row is not None
  attempts, state = row
  assert attempts >= 1
  assert state == "dead"
