...
class TestInfisicalMonitoringService:
    ...
    async def test_collect_metrics_success(self, monitoring_service: InfisicalMonitoringService) -> None:
        """Test successful metrics collection."""
        with patch.object(monitoring_service.infisical_client, 'health_check') as mock_client_health, \
            ...
            monitoring_service.redis.info = AsyncMock(return_value={
                "redis_version": "7.0.0",
            ...
            monitoring_service.key_manager.get_current_private_key = AsyncMock(return_value="current_key")
            ...
            monitoring_service.key_manager.check_rotation_needed = AsyncMock(return_value=(False, None))
            monitoring_service.key_manager._check_aes_rotation_needed = AsyncMock(return_value=(False, None))
            monitoring_service.key_manager._get_last_rotation_time = AsyncMock(return_value=datetime.now(UTC))
    ...
    async def test_collect_metrics_with_unhealthy_components(
        self, 
        monitoring_service: InfisicalMonitoringService
    ) -> None:
        ...
        with patch.object(monitoring_service.infisical_client, 'health_check') as mock_client_health, \
            ...
            mock_key_health.return_value = {"overall_status": "unhealthy", "errors": ["Key not found"]}
            
            # Mock Redis ping failure
            monitoring_service.redis.ping = AsyncMock(side_effect=Exception("Redis connection failed"))
    ...
    async def test_health_check_caching(self, monitoring_service: InfisicalMonitoringService) -> None:
    ...
    async def test_rotation_compliance_checking(self, monitoring_service: InfisicalMonitoringService) -> None:
        """Test key rotation compliance checking."""
        # Mock overdue rotation
        overdue_time = datetime.now(UTC) - timedelta(days=35)
        monitoring_service.key_manager._get_last_rotation_time = AsyncMock(return_value=overdue_time)
    ...
    async def test_alert_generation_and_storage(self, monitoring_service: InfisicalMonitoringService) -> None:
        ...
        metrics = {
            ...
            "security": {
                "jwt_keys": {"keys_properly_configured": False}
            }
        }
    ...
    async def test_metrics_history_retrieval(self, monitoring_service: InfisicalMonitoringService) -> None:
    ...
    async def test_cache_performance_testing(self, monitoring_service: InfisicalMonitoringService) -> None:
    ...
    async def test_webhook_metrics_collection(self, monitoring_service: InfisicalMonitoringService) -> None:
