...
async def test_jwks_headers(monkeypatch) -> None:
    ...
    body = await route_get_jwks(
        response=response, if_none_match=None, session=fake_session, redis=fake_redis
    )
    ...
    body2 = await route_get_jwks(
        response=response2, if_none_match=etag, session=fake_session, redis=fake_redis
    )
