		...
		return {
			...
			update(e, t) {
				if (
					...
					let t = i(e);
					if (o ? !En(t, e.values[s], n) : !n(t, e.values[s]))
...
const _s = /Edge\/(\d+)/.exec(Zs.userAgent),
	...
	js = "webkitFontSmoothing" in Ts.documentElement.style,
	Es = !Xs && /Apple Computer/.test(Zs.vendor),
	qs = Es && (/Mobile\/\w+/.test(Zs.userAgent) || Zs.maxTouchPoints > 2);
var Vs = {
	...
	updateViewportLines() {
		(this.viewportLines = []),
			this.heightMap.forEachLine(
				...
				0,
				(e) => {
					this.viewportLines.push(jl(e, this.scaler));
				},
			);
	}
	update(e, t = null) {
...
function jl(e, t) {
	...
	return new Ol(
		...
		i,
		n - i,
		Array.isArray(e._content) ? e._content.map((e) => jl(e, t)) : e._content,
	);
}
const El = Mn.define({ combine: (e) => e.join(" ") }),
...
class Fl {
	...
	update(e) {
		...
		if (
			...
			for (let e of this.state.facet(Eo))
				try {
					e(t);
				} catch (e) {
	...
	measure(e = !0) {
		...
		if (t && !t.empty) for (let e of this.state.facet(Eo)) e(t);
	}
	get themeClasses() {
		return (
			Vl + " " + (this.state.facet(ql) ? Wl : Ll) + " " + this.state.facet(El)
		);
	}
	updateAttrs() {
		let e = th(this, ea, {
			...
			constructor(t) {
				(this.view = t),
					(this.placeholder = to.set([
						to.widget({ widget: new jh(e), side: 1 }).range(0),
					]));
			}
			get decorations() {
...
class ju {
	...
	apply(e) {
		...
		return t.work(20, i) || t.takeTree(), new ju(t);
	}
	static init(e) {
		let t = Math.min(3e3, e.doc.length),
			i = Mu.create(e.facet(Lu).parser, e, { from: 0, to: t });
		return i.work(20, t) || i.takeTree(), new ju(i);
	}
}
Zu.state = Wn.define({
	create: ju.init,
			...
			work(e) {
				...
				(this.chunkBudget -= Date.now() - t),
					(a || this.chunkBudget <= 0) &&
						(r.context.takeTree(),
						this.view.dispatch({ effects: Zu.setState.of(new ju(r.context)) })),
...
function jf(e, t, i, n, r) {
	...
	for (let s = t.children.length - 1; s >= 0; s--) {
		let o = t.children[s],
			a = i + t.positions[s],
			l = o instanceof mc && a < r && jf(e, o, a, n, r);
		if (l) return l;
	}
	return null;
}
			...
			{ state: o, tree: a } = (function (e, t, i, n, r) {
				for (let r of t) {
					let t,
						...
						a = s <= i && o > i && jf(e, r.tree, 0 - r.offset, i, o);
					if (
						a &&
						a.pos <= n &&
						(t = Ef(e, r.tree, i + r.offset, a.pos + r.offset, !1))
					)
	...
	kO = [
		{
			key: "Alt-ArrowLeft",
			mac: "Ctrl-ArrowLeft",
			run: (e) => Td(e, (t) => Ed(e.state, t, !Ad(e))),
			shift: (e) => Id(e, (t) => Ed(e.state, t, !Ad(e))),
		},
		{
			key: "Alt-ArrowRight",
			mac: "Ctrl-ArrowRight",
...
const jO = cr.define({ map: (e, t) => e && e.map(t) }),
	EO = cr.define(),
	qO = Wn.define({
		create: () => null,
		update(e, t) {
			for (let i of t.effects) {
				if (i.is(jO)) return i.value;
				if (i.is(EO) && e) return new RO(e.ranges, i.value);
			}
			return (
		...
		return (
			i(
				t.update({
					...
					effects: jO.of(s ? null : new RO(n.ranges, r)),
					scrollIntoView: !0,
				}),
			),
			!0
...
const DO = [
		{ key: "Tab", run: WO(1), shift: WO(-1) },
		{
			key: "Escape",
			run: ({ state: e, dispatch: t }) =>
				!!e.field(qO, !1) && (t(e.update({ effects: jO.of(null) })), !0),
		},
	],
				...
				if (1 != this.stage) {
					if (2 == this.stage) {
						...
						if (t > this.pos) {
							...
							if (n) {
								let t = jp(e, n.to - this.start);
								t > 0 && (this.nextStage(n), (i = t));
							}
						}
						return i || (i = jp(e, this.pos)), i > 0 && i < e.length ? i : -1;
					}
					return jp(e, this.pos);
				}
...
const Em = {
		...
		parseInline: [
			{ name: "Superscript", parse: jm(94, "Superscript", "SuperscriptMark") },
		],
	},
	qm = {
		defineNodes: [
		...
		parseInline: [
			{ name: "Subscript", parse: jm(126, "Subscript", "SubscriptMark") },
		],
	},
	Vm = {
...
function qg(e = [], t = []) {
	...
	return qc((e, t) => {
		let a = e.type.id;
		if (28 == a) return Eg(e, t, i);
		if (31 == a) return Eg(e, t, n);
		if (34 == a) return Eg(e, t, r);
		if (20 == a && s.length) {
			let i,
				n = e.node,
				r = n.firstChild,
		...
		if (o && 23 == a) {
			...
			if ((i = n.firstChild)) {
				...
				if (e)
					for (let i of e) {
						if (i.tagName && i.tagName != jg(n.parent, t)) continue;
						let e = n.lastChild;
						if (26 == e.type.id) {
							let t = e.from + 1,
								n = e.lastChild,
...
const zb = ({ state: e, dispatch: t }) => {
	let i = Cu(e),
		...
		r = null,
		s = e.changeByRange((t) => {
			if (!t.empty || !Eb.isActiveAt(e, t.from, 0)) return (r = { range: t });
			let s = t.from,
				o = n.lineAt(s),
...
const Ib = ({ state: e, dispatch: t }) => {
		let i = Cu(e),
			...
			r = e.changeByRange((t) => {
				let r = t.from,
					{ doc: s } = e;
				if (t.empty && Eb.isActiveAt(e, t.from)) {
					let t = s.lineAt(r),
						n = Vb(
							(function (e, t) {
...
function ty(e) {
	return import("./chunks/index.BSdVDCNc.js").then((t) =>
		t.sql({ dialect: t[e] }),
	);
}
const iy = [
		Du.of({
			name: "C",
			extensions: ["c", "h", "ino"],
			load: () => import("./chunks/index.C1pJYXlK.js").then((e) => e.cpp()),
		}),
		Du.of({
			name: "C++",
			alias: ["cpp"],
		...
		Du.of({
			name: "Go",
			extensions: ["go"],
			load: () => import("./chunks/index.DrxWm3Ps.js").then((e) => e.go()),
		}),
		Du.of({
			name: "HTML",
			alias: ["xhtml"],
		...
		Du.of({
			name: "Java",
			extensions: ["java"],
			load: () => import("./chunks/index.BmhrnRM6.js").then((e) => e.java()),
		}),
		Du.of({
			name: "JavaScript",
		...
		Du.of({
			...
			alias: ["json5"],
			extensions: ["json", "map"],
			load: () => import("./chunks/index.IaIQQaGb.js").then((e) => e.json()),
		}),
		Du.of({
			name: "JSX",
			extensions: ["jsx"],
		...
		Du.of({
			name: "LESS",
			extensions: ["less"],
			load: () => import("./chunks/index.BdSlSSgF.js").then((e) => e.less()),
		}),
		Du.of({
			name: "Liquid",
			extensions: ["liquid"],
			load: () => import("./chunks/index.CR2B3QmS.js").then((e) => e.liquid()),
		}),
		...
		Du.of({
			...
			load: () => import("./chunks/index.8HPJ5zXW.js").then((e) => e.php()),
		}),
		...
		Du.of({
			...
			filename: /^(BUCK|BUILD)$/,
			load: () => import("./chunks/index.CD72PAiS.js").then((e) => e.python()),
		}),
		Du.of({
			name: "Rust",
			extensions: ["rs"],
			load: () => import("./chunks/index.C4swwE1S.js").then((e) => e.rust()),
		}),
		Du.of({
			name: "Sass",
			extensions: ["sass"],
			load: () =>
				import("./chunks/index.CM-ZvgvH.js").then((e) =>
					e.sass({ indented: !0 }),
				),
		}),
		Du.of({
			name: "SCSS",
			extensions: ["scss"],
			load: () => import("./chunks/index.CM-ZvgvH.js").then((e) => e.sass()),
		}),
		...
		Du.of({
			name: "WebAssembly",
			extensions: ["wat", "wast"],
			load: () => import("./chunks/index.bAg-Ne0T.js").then((e) => e.wast()),
		}),
		Du.of({
			name: "XML",
			alias: ["rss", "wsdl", "xsd"],
			extensions: ["xml", "xsl", "xsd", "svg"],
			load: () => import("./chunks/index.BV-p4fpD.js").then((e) => e.xml()),
		}),
		Du.of({
			name: "YAML",
			alias: ["yml"],
			extensions: ["yaml", "yml"],
			load: () => import("./chunks/index.DUiwjvLv.js").then((e) => e.yaml()),
		}),
		Du.of({
			name: "APL",
			extensions: ["dyalog", "apl"],
			load: () => import("./chunks/apl.UiYUtzS3.js").then((e) => ey(e.apl)),
		}),
		Du.of({
			name: "PGP",
			alias: ["asciiarmor"],
			...
			load: () =>
				import("./chunks/asciiarmor.vcwYhEDO.js").then((e) => ey(e.asciiArmor)),
		}),
		Du.of({
			name: "ASN.1",
			extensions: ["asn", "asn1"],
			load: () =>
				import("./chunks/asn1.CrI4yHHW.js").then((e) => ey(e.asn1({}))),
		}),
		Du.of({
			name: "Asterisk",
			filename: /^extensions\.conf$/i,
			load: () =>
				import("./chunks/asterisk.Bd8gaImV.js").then((e) => ey(e.asterisk)),
		}),
		Du.of({
			name: "Brainfuck",
			extensions: ["b", "bf"],
			load: () =>
				import("./chunks/brainfuck.CNuetEuz.js").then((e) => ey(e.brainfuck)),
		}),
		Du.of({
			name: "Cobol",
			extensions: ["cob", "cpy"],
			load: () => import("./chunks/cobol.5Vf7BReg.js").then((e) => ey(e.cobol)),
		}),
		Du.of({
			name: "C#",
			alias: ["csharp", "cs"],
			extensions: ["cs"],
			load: () =>
				import("./chunks/clike.BwGgP-c0.js").then((e) => ey(e.csharp)),
		}),
		Du.of({
			name: "Clojure",
			extensions: ["clj", "cljc", "cljx"],
			load: () =>
				import("./chunks/clojure.C4BtSI05.js").then((e) => ey(e.clojure)),
		}),
		Du.of({
			name: "ClojureScript",
			extensions: ["cljs"],
		...
		Du.of({
			...
			extensions: ["gss"],
			load: () => import("./chunks/css.dcnMAPCH.js").then((e) => ey(e.gss)),
		}),
		Du.of({
			name: "CMake",
			extensions: ["cmake", "cmake.in"],
			filename: /^CMakeLists\.txt$/,
			load: () => import("./chunks/cmake.BnOACmyy.js").then((e) => ey(e.cmake)),
		}),
		Du.of({
			name: "CoffeeScript",
			...
			extensions: ["coffee"],
			load: () =>
				import("./chunks/coffeescript.BU6rJgRK.js").then((e) =>
					ey(e.coffeeScript),
				),
		}),
		Du.of({
			...
			load: () =>
				import("./chunks/commonlisp.DZbnmI67.js").then((e) => ey(e.commonLisp)),
		}),
		Du.of({
			name: "Cypher",
			extensions: ["cyp", "cypher"],
			load: () =>
				import("./chunks/cypher.DlHjAYrz.js").then((e) => ey(e.cypher)),
		}),
		Du.of({
			name: "Cython",
			extensions: ["pyx", "pxd", "pxi"],
			load: () =>
				import("./chunks/python.ClQam6iS.js").then((e) => ey(e.cython)),
		}),
		Du.of({
			name: "Crystal",
			extensions: ["cr"],
			load: () =>
				import("./chunks/crystal.qNGrqJEP.js").then((e) => ey(e.crystal)),
		}),
		Du.of({
			name: "D",
			extensions: ["d"],
			load: () => import("./chunks/d.g1JImozx.js").then((e) => ey(e.d)),
		}),
		Du.of({
			name: "Dart",
			extensions: ["dart"],
			load: () => import("./chunks/clike.BwGgP-c0.js").then((e) => ey(e.dart)),
		}),
		Du.of({
			name: "diff",
			extensions: ["diff", "patch"],
			load: () => import("./chunks/diff.DkK85mZX.js").then((e) => ey(e.diff)),
		}),
		Du.of({
			name: "Dockerfile",
			filename: /^Dockerfile$/,
			load: () =>
				import("./chunks/dockerfile.gLkp8t8E.js").then((e) => ey(e.dockerFile)),
		}),
		Du.of({
			name: "DTD",
			extensions: ["dtd"],
			load: () => import("./chunks/dtd.C964Odv5.js").then((e) => ey(e.dtd)),
		}),
		Du.of({
			name: "Dylan",
			extensions: ["dylan", "dyl", "intr"],
			load: () => import("./chunks/dylan.Bo7uv6Nm.js").then((e) => ey(e.dylan)),
		}),
		Du.of({
			name: "EBNF",
			load: () => import("./chunks/ebnf.Df5sro3Y.js").then((e) => ey(e.ebnf)),
		}),
		Du.of({
			name: "ECL",
			extensions: ["ecl"],
			load: () => import("./chunks/ecl.DYa78ZvT.js").then((e) => ey(e.ecl)),
		}),
		Du.of({
			name: "edn",
			extensions: ["edn"],
		...
		Du.of({
			...
			extensions: ["e"],
			load: () =>
				import("./chunks/eiffel.Ddp73B1n.js").then((e) => ey(e.eiffel)),
		}),
		Du.of({
			name: "Elm",
			extensions: ["elm"],
			load: () => import("./chunks/elm.BCXZKtim.js").then((e) => ey(e.elm)),
		}),
		Du.of({
			name: "Erlang",
			extensions: ["erl"],
			load: () =>
				import("./chunks/erlang.Bsnwu7iN.js").then((e) => ey(e.erlang)),
		}),
		Du.of({
			name: "Esper",
			load: () => import("./chunks/sql.B4yYPsou.js").then((e) => ey(e.esper)),
		}),
		Du.of({
			name: "Factor",
			extensions: ["factor"],
			load: () =>
				import("./chunks/factor.BLaFcoK3.js").then((e) => ey(e.factor)),
		}),
		Du.of({
			name: "FCL",
			load: () => import("./chunks/fcl.Bh3tvLxP.js").then((e) => ey(e.fcl)),
		}),
		Du.of({
			name: "Forth",
			extensions: ["forth", "fth", "4th"],
			load: () => import("./chunks/forth.hmpV37AG.js").then((e) => ey(e.forth)),
		}),
		Du.of({
			name: "Fortran",
			extensions: ["f", "for", "f77", "f90", "f95"],
			load: () =>
				import("./chunks/fortran.B_4JPl-e.js").then((e) => ey(e.fortran)),
		}),
		Du.of({
			name: "F#",
			alias: ["fsharp"],
			extensions: ["fs"],
			load: () =>
				import("./chunks/mllike.D2ExWDQd.js").then((e) => ey(e.fSharp)),
		}),
		Du.of({
			name: "Gas",
			extensions: ["s"],
			load: () => import("./chunks/gas.yF4d7mHb.js").then((e) => ey(e.gas)),
		}),
		Du.of({
			name: "Gherkin",
			extensions: ["feature"],
			load: () =>
				import("./chunks/gherkin.BcqyBRu1.js").then((e) => ey(e.gherkin)),
		}),
		Du.of({
			name: "Groovy",
			...
			load: () =>
				import("./chunks/groovy.DTjkXoVf.js").then((e) => ey(e.groovy)),
		}),
		Du.of({
			name: "Haskell",
			extensions: ["hs"],
			load: () =>
				import("./chunks/haskell.Bqhxn4lK.js").then((e) => ey(e.haskell)),
		}),
		Du.of({
			name: "Haxe",
			extensions: ["hx"],
			load: () => import("./chunks/haxe.DUXrYZ_8.js").then((e) => ey(e.haxe)),
		}),
		Du.of({
			name: "HXML",
			extensions: ["hxml"],
			load: () => import("./chunks/haxe.DUXrYZ_8.js").then((e) => ey(e.hxml)),
		}),
		Du.of({
			name: "HTTP",
			load: () => import("./chunks/http.DqJ0cLyU.js").then((e) => ey(e.http)),
		}),
		Du.of({
			name: "IDL",
			extensions: ["pro"],
			load: () => import("./chunks/idl.B8ognL-6.js").then((e) => ey(e.idl)),
		}),
		Du.of({
			name: "JSON-LD",
			alias: ["jsonld"],
			extensions: ["jsonld"],
			load: () =>
				import("./chunks/javascript.DSlxvpdh.js").then((e) => ey(e.jsonld)),
		}),
		Du.of({
			name: "Jinja2",
			extensions: ["j2", "jinja", "jinja2"],
			load: () =>
				import("./chunks/jinja2.BQ3uRo_g.js").then((e) => ey(e.jinja2)),
		}),
		Du.of({
			name: "Julia",
			extensions: ["jl"],
			load: () => import("./chunks/julia.BA5fD2-1.js").then((e) => ey(e.julia)),
		}),
		Du.of({
			name: "Kotlin",
			extensions: ["kt", "kts"],
			load: () =>
				import("./chunks/clike.BwGgP-c0.js").then((e) => ey(e.kotlin)),
		}),
		Du.of({
			name: "LiveScript",
			alias: ["ls"],
			extensions: ["ls"],
			load: () =>
				import("./chunks/livescript.sn7Ebe3I.js").then((e) => ey(e.liveScript)),
		}),
		Du.of({
			name: "Lua",
			extensions: ["lua"],
			load: () => import("./chunks/lua.5I0rLdEw.js").then((e) => ey(e.lua)),
		}),
		Du.of({
			name: "mIRC",
			extensions: ["mrc"],
			load: () => import("./chunks/mirc.CLGj5Ry4.js").then((e) => ey(e.mirc)),
		}),
		Du.of({
			name: "Mathematica",
			extensions: ["m", "nb", "wl", "wls"],
			load: () =>
				import("./chunks/mathematica.D_uGPb_I.js").then((e) =>
					ey(e.mathematica),
				),
		}),
		Du.of({
			...
			extensions: ["mo"],
			load: () =>
				import("./chunks/modelica.DpGFg-JJ.js").then((e) => ey(e.modelica)),
		}),
		Du.of({
			name: "MUMPS",
			extensions: ["mps"],
			load: () => import("./chunks/mumps.YDBcz52_.js").then((e) => ey(e.mumps)),
		}),
		Du.of({
			name: "Mbox",
			extensions: ["mbox"],
			load: () => import("./chunks/mbox.BfvmtAJV.js").then((e) => ey(e.mbox)),
		}),
		Du.of({
			name: "Nginx",
			filename: /nginx.*\.conf$/i,
			load: () => import("./chunks/nginx.B00S42UB.js").then((e) => ey(e.nginx)),
		}),
		Du.of({
			name: "NSIS",
			extensions: ["nsh", "nsi"],
			load: () => import("./chunks/nsis.DPekJ8Dz.js").then((e) => ey(e.nsis)),
		}),
		Du.of({
			name: "NTriples",
			extensions: ["nt", "nq"],
			load: () =>
				import("./chunks/ntriples.DXulM8i7.js").then((e) => ey(e.ntriples)),
		}),
		Du.of({
			name: "Objective-C",
			...
			extensions: ["m"],
			load: () =>
				import("./chunks/clike.BwGgP-c0.js").then((e) => ey(e.objectiveC)),
		}),
		Du.of({
			name: "Objective-C++",
			alias: ["objective-c++", "objc++"],
			extensions: ["mm"],
			load: () =>
				import("./chunks/clike.BwGgP-c0.js").then((e) => ey(e.objectiveCpp)),
		}),
		Du.of({
			name: "OCaml",
			extensions: ["ml", "mli", "mll", "mly"],
			load: () =>
				import("./chunks/mllike.D2ExWDQd.js").then((e) => ey(e.oCaml)),
		}),
		Du.of({
			name: "Octave",
			extensions: ["m"],
			load: () =>
				import("./chunks/octave.Sg9cMShp.js").then((e) => ey(e.octave)),
		}),
		Du.of({
			name: "Oz",
			extensions: ["oz"],
			load: () => import("./chunks/oz.BPiTtpNU.js").then((e) => ey(e.oz)),
		}),
		Du.of({
			name: "Pascal",
			extensions: ["p", "pas"],
			load: () =>
				import("./chunks/pascal.B7SZGK9e.js").then((e) => ey(e.pascal)),
		}),
		Du.of({
			name: "Perl",
			extensions: ["pl", "pm"],
			load: () => import("./chunks/perl.v0qHaK9M.js").then((e) => ey(e.perl)),
		}),
		Du.of({
			name: "Pig",
			extensions: ["pig"],
			load: () => import("./chunks/pig.Q_m4RMb7.js").then((e) => ey(e.pig)),
		}),
		Du.of({
			name: "PowerShell",
			extensions: ["ps1", "psd1", "psm1"],
			load: () =>
				import("./chunks/powershell.BFFq7p8d.js").then((e) => ey(e.powerShell)),
		}),
		Du.of({
			...
			load: () =>
				import("./chunks/properties.D4n84Z7s.js").then((e) => ey(e.properties)),
		}),
		Du.of({
			name: "ProtoBuf",
			extensions: ["proto"],
			load: () =>
				import("./chunks/protobuf.yyCbt-jz.js").then((e) => ey(e.protobuf)),
		}),
		Du.of({
			name: "Pug",
			alias: ["jade"],
			extensions: ["pug", "jade"],
			load: () => import("./chunks/pug.BolkvwLI.js").then((e) => ey(e.pug)),
		}),
		Du.of({
			name: "Puppet",
			extensions: ["pp"],
			load: () =>
				import("./chunks/puppet.Oo1hXguV.js").then((e) => ey(e.puppet)),
		}),
		Du.of({
			name: "Q",
			extensions: ["q"],
			load: () => import("./chunks/q.DOW78MF0.js").then((e) => ey(e.q)),
		}),
		Du.of({
			name: "R",
			alias: ["rscript"],
			extensions: ["r", "R"],
			load: () => import("./chunks/r.ikhzD2xI.js").then((e) => ey(e.r)),
		}),
		Du.of({
			name: "RPM Changes",
			load: () =>
				import("./chunks/rpm.Bxmk5pEe.js").then((e) => ey(e.rpmChanges)),
		}),
		Du.of({
			name: "RPM Spec",
			extensions: ["spec"],
			load: () => import("./chunks/rpm.Bxmk5pEe.js").then((e) => ey(e.rpmSpec)),
		}),
		Du.of({
			name: "Ruby",
			...
			load: () => import("./chunks/ruby.BkBz5FF5.js").then((e) => ey(e.ruby)),
		}),
		Du.of({
			name: "SAS",
			extensions: ["sas"],
			load: () => import("./chunks/sas.CXSuW51O.js").then((e) => ey(e.sas)),
		}),
		Du.of({
			name: "Scala",
			extensions: ["scala"],
			load: () => import("./chunks/clike.BwGgP-c0.js").then((e) => ey(e.scala)),
		}),
		Du.of({
			name: "Scheme",
			extensions: ["scm", "ss"],
			load: () =>
				import("./chunks/scheme.sok-HOEx.js").then((e) => ey(e.scheme)),
		}),
		Du.of({
			name: "Shell",
			alias: ["bash", "sh", "zsh"],
			...
			filename: /^PKGBUILD$/,
			load: () => import("./chunks/shell.CuICg3JH.js").then((e) => ey(e.shell)),
		}),
		Du.of({
			name: "Sieve",
			extensions: ["siv", "sieve"],
			load: () => import("./chunks/sieve.BBi1ir5a.js").then((e) => ey(e.sieve)),
		}),
		Du.of({
			name: "Smalltalk",
			extensions: ["st"],
			load: () =>
				import("./chunks/smalltalk.B3HJIOo2.js").then((e) => ey(e.smalltalk)),
		}),
		Du.of({
			name: "Solr",
			load: () => import("./chunks/solr.m8BUjTS3.js").then((e) => ey(e.solr)),
		}),
		Du.of({
			name: "SML",
			extensions: ["sml", "sig", "fun", "smackspec"],
			load: () => import("./chunks/mllike.D2ExWDQd.js").then((e) => ey(e.sml)),
		}),
		Du.of({
			name: "SPARQL",
			alias: ["sparul"],
			...
			load: () =>
				import("./chunks/sparql.DpAaQO6m.js").then((e) => ey(e.sparql)),
		}),
		Du.of({
			name: "Spreadsheet",
			alias: ["excel", "formula"],
			load: () =>
				import("./chunks/spreadsheet.CBjFgby7.js").then((e) =>
					ey(e.spreadsheet),
				),
		}),
		Du.of({
			...
			extensions: ["nut"],
			load: () =>
				import("./chunks/clike.BwGgP-c0.js").then((e) => ey(e.squirrel)),
		}),
		Du.of({
			name: "Stylus",
			extensions: ["styl"],
			load: () =>
				import("./chunks/stylus.BAECfY1D.js").then((e) => ey(e.stylus)),
		}),
		Du.of({
			name: "Swift",
			extensions: ["swift"],
			load: () => import("./chunks/swift.D3uVdhZA.js").then((e) => ey(e.swift)),
		}),
		Du.of({
			name: "sTeX",
			load: () => import("./chunks/stex.B9UJhNZZ.js").then((e) => ey(e.stex)),
		}),
		Du.of({
			name: "LaTeX",
			alias: ["tex"],
		...
		Du.of({
			...
			extensions: ["v", "sv", "svh"],
			load: () =>
				import("./chunks/verilog.BfSF5igT.js").then((e) => ey(e.verilog)),
		}),
		Du.of({
			name: "Tcl",
			extensions: ["tcl"],
			load: () => import("./chunks/tcl.DX5cHFLN.js").then((e) => ey(e.tcl)),
		}),
		Du.of({
			name: "Textile",
			extensions: ["textile"],
			load: () =>
				import("./chunks/textile.B1UOALr3.js").then((e) => ey(e.textile)),
		}),
		Du.of({
			name: "TiddlyWiki",
			load: () =>
				import("./chunks/tiddlywiki.BmFpEzZr.js").then((e) => ey(e.tiddlyWiki)),
		}),
		Du.of({
			name: "Tiki wiki",
			load: () => import("./chunks/tiki.TcMSPPtB.js").then((e) => ey(e.tiki)),
		}),
		Du.of({
			name: "TOML",
			extensions: ["toml"],
			load: () => import("./chunks/toml.DZL5JzPK.js").then((e) => ey(e.toml)),
		}),
		Du.of({
			name: "Troff",
			extensions: ["1", "2", "3", "4", "5", "6", "7", "8", "9"],
			load: () => import("./chunks/troff.DYcacBPD.js").then((e) => ey(e.troff)),
		}),
		Du.of({
			name: "TTCN",
			extensions: ["ttcn", "ttcn3", "ttcnpp"],
			load: () => import("./chunks/ttcn.DO2nd46F.js").then((e) => ey(e.ttcn)),
		}),
		Du.of({
			name: "TTCN_CFG",
			extensions: ["cfg"],
			load: () =>
				import("./chunks/ttcn-cfg.SEFT7isw.js").then((e) => ey(e.ttcnCfg)),
		}),
		Du.of({
			name: "Turtle",
			extensions: ["ttl"],
			load: () =>
				import("./chunks/turtle.Dn4mcvk8.js").then((e) => ey(e.turtle)),
		}),
		Du.of({
			name: "Web IDL",
			extensions: ["webidl"],
			load: () =>
				import("./chunks/webidl.BI_Xvm8b.js").then((e) => ey(e.webIDL)),
		}),
		Du.of({
			name: "VB.NET",
			extensions: ["vb"],
			load: () => import("./chunks/vb.DzXVEGXN.js").then((e) => ey(e.vb)),
		}),
		Du.of({
			name: "VBScript",
			extensions: ["vbs"],
			load: () =>
				import("./chunks/vbscript.DzYxhDC0.js").then((e) => ey(e.vbScript)),
		}),
		Du.of({
			name: "Velocity",
			extensions: ["vtl"],
			load: () =>
				import("./chunks/velocity.Cea6c5J4.js").then((e) => ey(e.velocity)),
		}),
		Du.of({
			name: "Verilog",
			extensions: ["v"],
		...
		Du.of({
			name: "VHDL",
			extensions: ["vhd", "vhdl"],
			load: () => import("./chunks/vhdl.BI_dj9sH.js").then((e) => ey(e.vhdl)),
		}),
		Du.of({
			name: "XQuery",
			extensions: ["xy", "xquery"],
			load: () =>
				import("./chunks/xquery.qUPs2mS6.js").then((e) => ey(e.xQuery)),
		}),
		Du.of({
			name: "Yacas",
			extensions: ["ys"],
			load: () => import("./chunks/yacas.D_afzyX7.js").then((e) => ey(e.yacas)),
		}),
		Du.of({
			name: "Z80",
			extensions: ["z80"],
			load: () => import("./chunks/z80.fGCHUei_.js").then((e) => ey(e.z80)),
		}),
		Du.of({
			name: "MscGen",
			extensions: ["mscgen", "mscin", "msc"],
			load: () =>
				import("./chunks/mscgen.DrjBuP0F.js").then((e) => ey(e.mscgen)),
		}),
		Du.of({
			name: "Xù",
			extensions: ["xu"],
			load: () => import("./chunks/mscgen.DrjBuP0F.js").then((e) => ey(e.xu)),
		}),
		Du.of({
			name: "MsGenny",
			extensions: ["msgenny"],
			load: () =>
				import("./chunks/mscgen.DrjBuP0F.js").then((e) => ey(e.msgenny)),
		}),
		Du.of({
			name: "Vue",
			extensions: ["vue"],
			load: () => import("./chunks/index.BYy19WIb.js").then((e) => e.vue()),
		}),
		Du.of({
			name: "Angular Template",
			load: () => import("./chunks/index.BECnPECY.js").then((e) => e.angular()),
		}),
	],
	...
	const { onChange: n = () => {}, additionalExtensions: r = [] } = i,
		s = wr.create({
			...
			extensions: [
				...
				Fl.lineWrapping,
				sh.of([...ip, ...kO, ...$d]),
				Gb({ base: Eb, codeLanguages: iy }),
				my,
				Eh("Start writing your journal entry..."),
				gy(n),
				...r,
			],
		});
