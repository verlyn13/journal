var e = function (e, t) {
		...
		return e.match((a = x[n])) && a.regex && e.match(a.regex)
	...
	x = {
		start: [
			...
			{ token: "variableName", regex: t + "\\s*:(?![:=])" },
			{ token: "variableName", regex: t },
		...
		key: [
			...
			{ token: "variableName", regex: t, next: "start" },
...
for (var a in x) {
	var g = x[a];
	if (g.splice)
		for (var s = 0, i = g.length; s < i; ++s) {
			var k = g[s];
			"string" == typeof k.regex && (x[a][s].regex = new RegExp("^" + k.regex));
		}
	else "string" == typeof k.regex && (x[a].regex = new RegExp("^" + g.regex));
}
const l = {
	name: "livescript",
	startState: function () {
