...
function t(t, n, r, a) {
	...
	return (
		...
		(t.context = new e(o, n, r, a, null, t.context))
	);
}
function n(e) {
...
function o(o) {
	var i,
		...
		y = o.atoms || {},
		g = o.hooks || {},
		k = o.multiLineStrings,
		b = !1 !== o.indentStatements,
		v = !1 !== o.indentSwitch,
		w = o.namespaceSeparator,
		_ = o.isPunctuationChar || /[\[\]{}\(\),;\:\.]/,
		x = o.numberStart || /[\d\.]/,
		S =
			o.number ||
			/^(?:0x[a-f\d]+|0b[01]+|(?:\d+\.?\d*|\.\d+)(?:e[-+]?\d+)?)(u|ll?|l|f)?/i,
		T = o.isOperatorChar || /[+\-*&%=<>!?|\/]/,
		N = o.isIdentifierChar || /[\w\$_\xa1-\uffff]/,
		I = o.isReservedIdentifier || !1;
	function D(e, t) {
		var n,
			r = e.next();
		if (g[r]) {
			...
			return (
				(t.tokenize =
					...
					function (e, t) {
						...
						return (o || (!a && !k)) && (t.tokenize = null), "string";
					})),
				t.tokenize(e, t)
			);
		if (x.test(r)) {
		...
		if ((e.eatWhile(N), w)) for (; e.match(w); ) e.eatWhile(N);
		var o = e.current();
		return l(d, o)
			? (l(m, o) && (i = "newstatement"), l(h, o) && (s = !0), "keyword")
			: l(f, o)
				? "type"
				: l(p, o) || (I && I(o))
					? (l(m, o) && (i = "newstatement"), "builtin")
					: l(y, o)
						? "atom"
						: "variable";
	}
	...
	return {
		...
		token: function (e, l) {
			...
			if (
				(e.sol() &&
					...
					(l.indented = e.indentation()),
					(l.startOfLine = !0)),
				e.eatSpace())
			)
				return z(e, l), null;
			i = s = null;
			var u = (l.tokenize || D)(e, l);
			if ("comment" == u || "meta" == u) return u;
			if (
				(null == c.align && (c.align = !0),
				";" == i || ":" == i || ("," == i && e.match(/^\s*(?:\/\/.*)?$/, !1)))
			)
				for (; "statement" == l.context.type; ) n(l);
			else if ("{" == i) t(l, e.column(), "}");
			else if ("[" == i) t(l, e.column(), "]");
			else if ("(" == i) t(l, e.column(), ")");
			else if ("}" == i) {
				for (; "statement" == c.type; ) c = n(l);
				for ("}" == c.type && (c = n(l)); "statement" == c.type; ) c = n(l);
			} else
				i == c.type
					? n(l)
					: b &&
						((("}" == c.type || "top" == c.type) && ";" != i) ||
							("statement" == c.type && "newstatement" == i)) &&
						t(l, e.column(), "statement", e.current());
			if (
				("variable" == u &&
					("def" == l.prevToken ||
						(o.typeFirstDefinitions &&
			...
			return (
				"def" == u && !1 === o.styleDefs && (u = "variable"),
				(l.startOfLine = !1),
				(l.prevToken = s ? "def" : u || i),
				z(e, l),
				u
			);
		},
		indent: function (e, t, n) {
			if (
				(e.tokenize != D && null != e.tokenize) ||
				(e.typeAtEndOfLine && a(e.context))
			)
				return null;
			var r = e.context,
				i = t && t.charAt(0),
			...
			if (
				...
				o.dontIndentStatements)
			)
				for (; "statement" == r.type && o.dontIndentStatements.test(r.info); )
					r = r.prev;
			if (g.indent) {
				var s = g.indent(e, r, t, n.unit);
				if ("number" == typeof s) return s;
			}
			var d = r.prev && "switch" == r.prev.info;
			if (o.allmanIndentation && /[{(]/.test(i)) {
				for (; "top" != r.type && "}" != r.type; ) r = r.prev;
				return r.indented;
			}
			return "statement" == r.type
				? r.indented + ("{" == i ? 0 : c || n.unit)
				: !r.align || (u && ")" == r.type)
					? ")" != r.type || l
						? r.indented +
							(l ? 0 : n.unit) +
							(l || !d || /^(?:case|default)\b/.test(t) ? 0 : n.unit)
						: r.indented + (c || n.unit)
					: r.column + (l ? 0 : 1);
		},
		languageData: {
...
const N = o({
	...
	I = o({
		...
		defKeywords: i(g + " class namespace"),
		typeFirstDefinitions: !0,
	...
	D = o({
		...
		hooks: {
			...
			'"': function (e, t) {
				return !!e.match(/""$/) && ((t.tokenize = z), t.tokenize(e, t));
			},
		},
	}),
	C = o({
...
const E = o({
	...
	R = o({
		...
		defKeywords: i(g + " @interface @implementation @protocol @class"),
	...
	O = o({
		...
		defKeywords: i(
			g + " @interface @implementation @protocol @class class namespace",
		),
		dontIndentStatements: /^@.*$|^template$/,
		typeFirstDefinitions: !0,
		atoms: i("YES NO NULL Nil nil true false nullptr"),
...
const $ = o({
	...
	hooks: {
		...
		'"': function (e, t) {
			return (
				(t.tokenize = U(e.match('""') ? "triple" : "single")), t.tokenize(e, t)
			);
		},
		"`": function (e, t) {
			return (
				!(!j || !e.match("`")) &&
				((t.tokenize = j), (j = null), t.tokenize(e, t))
			);
		},
		"'": function (e) {
			return e.match(/^(\\[^'\s]+|[^\\'])'/)
...
function q(e, t, n, r) {
	...
	function o(t, n) {
		for (var o = !1; !t.eol(); ) {
			if (!r && !o && "$" == t.peek()) return B(n), (n.tokenize = V), "string";
			var i = t.next();
...
function V(e, t) {
	return e.eat("$"), e.eat("{") ? (t.tokenize = null) : (t.tokenize = W), null;
}
function W(e, t) {
	return e.eatWhile(/[\w_]/), (t.tokenize = K(t)), "variable";
}
const G = o({
	name: "dart",
	keywords: i(
