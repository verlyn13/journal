...
function c(e, t) {
	...
	if (e.eat(")")) return (t.bracketNesting -= 1), "punctuation";
	for (var o in u) if (e.match(u[o])) return o;
	var i = e.next();
	if ("'" === i)
		return (function (e, t) {
			var r;
			for (; null != (r = e.peek()); )
				if ((e.next(), "'" === r && !e.eat("'")))
					return (t.tokenize = c), "string";
			return "error";
		})(e, t);
	if ("$" === i) return g(e, t);
	if ('"' === i) return l(e, t);
	if ("<" === i && e.eat("#")) return (t.tokenize = P), P(e, t);
	if ("#" === i) return e.skipToEnd(), "comment";
	if ("@" === i) {
		var a = e.eat(/["']/);
		if (a && e.eol()) return (t.tokenize = v), (t.startQuote = a[0]), v(e, t);
		if (e.eol()) return "error";
		if (e.peek().match(/[({]/)) return "punctuation";
		if (e.peek().match(r)) return g(e, t);
	}
	return "error";
}
function l(e, t) {
	for (var r; null != (r = e.peek()); ) {
		if ("$" === r) return (t.tokenize = p), "string";
		if ((e.next(), "`" !== r)) {
			if ('"' === r && !e.eat('"')) return (t.tokenize = c), "string";
		} else e.next();
	}
	return "error";
}
...
function m(e, t) {
	return (t.tokenize = v), (t.startQuote = '"'), v(e, t);
}
function S(e, t) {
	return f(e, t, m);
}
...
function g(e, t) {
	...
	return e.eat("{")
		? ((t.tokenize = d), d(e, t))
		: null != n && n.match(r)
			? (e.eatWhile(r), (t.tokenize = c), "variable")
			: ((t.tokenize = c), "error");
}
function d(e, t) {
...
function v(e, t) {
	var r = t.startQuote;
	if (e.sol() && e.match(new RegExp(r + "@"))) t.tokenize = c;
	else if ('"' === r)
		for (; !e.eol(); ) {
			var n = e.peek();
...
const b = {
	name: "powershell",
	startState: function () {
		return { returnStack: [], bracketNesting: 0, tokenize: c };
	},
	token: function (e, t) {
