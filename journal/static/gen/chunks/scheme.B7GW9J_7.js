...
function d(e, t, n) {
	e.indentStack = new o(t, n, e.indentStack);
}
var u = new RegExp(
...
function x(e, t) {
	return !0 === t && e.backUp(1), e.match(f);
}
function b(e) {
	return e.match(p);
}
...
const k = {
	...
	token: function (s, o) {
		if (
			(null == o.indentStack && s.sol() && (o.indentation = s.indentation()),
			s.eatSpace())
		)
			return null;
		var u = null;
		switch (o.mode) {
			case "string":
				v(s, { token: '"', state: o }), (u = t);
				break;
			case "symbol":
				v(s, { token: "|", state: o }), (u = n);
				break;
			case "comment":
				for (var m, p = !1; null != (m = s.next()); ) {
					if ("#" == m && p) {
						o.mode = !1;
						break;
					}
			...
			default:
				var f = s.next();
				if ('"' == f) (o.mode = "string"), (u = t);
				else if ("'" == f)
					"(" == s.peek() || "[" == s.peek()
						? ("number" != typeof o.sExprQuote && (o.sExprQuote = 0), (u = r))
						: (s.eatWhile(/[\w_\-!$%&*+\.\/:<=>?@\^~]/), (u = r));
				else if ("|" == f) (o.mode = "symbol"), (u = n);
				else if ("#" == f)
					if (s.eat("|")) (o.mode = "comment"), (u = e);
					else if (s.eat(/[tf]/i)) u = r;
					else if (s.eat(";")) (o.mode = "s-expr-comment"), (u = e);
					else {
						var k = null,
							y = !1,
							w = !0;
						s.eat(/[ei]/i) ? (y = !0) : s.backUp(1),
							s.match(/^#b/i)
								? (k = h)
								: s.match(/^#o/i)
									? (k = g)
									: s.match(/^#x/i)
										...
										: s.match(/^#d/i)
											...
											: s.match(/^[-+0-9.]/, !1)
												? ((w = !1), (k = x))
												: y || s.eat("#"),
							null != k && (w && !y && s.match(/^#[ei]/i), k(s) && (u = a));
					}
				else if (/^[-+0-9.]/.test(f) && x(s, !0)) u = a;
				else if (";" == f) s.skipToEnd(), (u = e);
				else if ("(" == f || "[" == f) {
					for (
						var E, S = "", q = s.column();
						null != (E = s.eat(/[^\s\(\[\;\)\]]/));
						...
						S += E;
					S.length > 0 && l.propertyIsEnumerable(S)
						? d(o, q + 2, f)
						: (s.eatSpace(),
							s.eol() || ";" == s.peek()
								? d(o, q + 1, f)
								: d(o, q + s.current().length, f)),
						s.backUp(s.current().length - 1),
						"number" == typeof o.sExprComment && o.sExprComment++,
						"number" == typeof o.sExprQuote && o.sExprQuote++,
						(u = i);
				} else
					")" == f || "]" == f
						? ((u = i),
							null != o.indentStack &&
								o.indentStack.type == (")" == f ? "(" : "[") &&
								(!(function (e) {
									e.indentStack = e.indentStack.prev;
								})(o),
								"number" == typeof o.sExprComment &&
									0 == --o.sExprComment &&
									((u = e), (o.sExprComment = !1)),
								"number" == typeof o.sExprQuote &&
									0 == --o.sExprQuote &&
									((u = r), (o.sExprQuote = !1))))
						: (s.eatWhile(/[\w_\-!$%&*+\.\/:<=>?@\^~]/),
							(u =
								c && c.propertyIsEnumerable(s.current())
									? "builtin"
									: "variable"));
		}
		return "number" == typeof o.sExprComment
			? e
			: "number" == typeof o.sExprQuote
				? r
				: u;
