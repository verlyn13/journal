...
function s(t, n, r) {
	if (r)
		for (var s = n.split(" "), o = 0; o < s.length; o++)
			e[s[o]] = { style: t, state: r };
}
s("def", "stack pgm view source debug nesting nolist", ["inDataStep"]),
	s("def", "if while until for do do; end end; then else cancel", [
		"inDataStep",
	]),
	...
	token: function (s, o) {
		return s.eatSpace()
			...
			: (function (s, o) {
					var a = s.next();
					if ("/" === a && s.eat("*"))
						return (o.continueComment = !0), "comment";
					if (!0 === o.continueComment)
						return (
							"*" === a && "/" === s.peek()
								? (s.next(), (o.continueComment = !1))
								: s.skipTo("*")
									? (s.skipTo("*"),
										s.next(),
										s.eat("/") && (o.continueComment = !1))
									: s.skipToEnd(),
							"comment"
						);
					if ("*" == a && s.column() == s.indentation())
						return s.skipToEnd(), "comment";
					var i,
						l = a + s.peek();
					if (!(('"' !== a && "'" !== a) || o.continueString))
						return (o.continueString = a), "string";
					if (o.continueString)
						return (
							o.continueString == a
								? (o.continueString = null)
								: s.skipTo(o.continueString)
									? (s.next(), (o.continueString = null))
									: s.skipToEnd(),
							"string"
						);
					if (null !== o.continueString && s.eol())
						return s.skipTo(o.continueString) || s.skipToEnd(), "string";
					if (/[\d\.]/.test(a))
						return (
							"." === a
								? s.match(/^[0-9]+([eE][\-+]?[0-9]+)?/)
					...
					if (n.test(a + s.peek())) return s.next(), "operator";
					if (t.hasOwnProperty(l)) {
						if ((s.next(), " " === s.peek())) return t[l.toLowerCase()];
					} else if (r.test(a)) return "operator";
					if (null != s.match(/[%&;\w]+/, !1)) {
						if (((i = a + s.match(/[%&;\w]+/, !0)), /&/.test(i)))
							return "variable";
					} else i = a;
					if (o.nextword)
						return (
					...
					if (((i = i.toLowerCase()), o.inDataStep)) {
						if ("run;" === i || s.match(/run\s;/))
							return (o.inDataStep = !1), "builtin";
						if (i && "." === s.next())
							return /\w/.test(s.peek()) ? "variableName.special" : "variable";
						if (
							i &&
							e.hasOwnProperty(i) &&
							(-1 !== e[i].state.indexOf("inDataStep") ||
								-1 !== e[i].state.indexOf("ALL"))
						) {
							s.start < s.pos && s.backUp(s.pos - s.start);
							for (var c = 0; c < i.length; ++c) s.next();
							return e[i].style;
						}
					}
					if (o.inProc) {
						if ("run;" === i || "quit;" === i)
							return (o.inProc = !1), "builtin";
						if (
							i &&
							e.hasOwnProperty(i) &&
							(-1 !== e[i].state.indexOf("inProc") ||
								-1 !== e[i].state.indexOf("ALL"))
						)
							return s.match(/[\w]+/), e[i].style;
					}
					return o.inMacro
						? "%mend" === i
							? (";" === s.peek() && s.next(), (o.inMacro = !1), "builtin")
							: i &&
									e.hasOwnProperty(i) &&
									(-1 !== e[i].state.indexOf("inMacro") ||
										-1 !== e[i].state.indexOf("ALL"))
								? (s.match(/[\w]+/), e[i].style)
								: "atom"
						: i && e.hasOwnProperty(i)
							? (s.backUp(1),
								s.match(/[\w]+/),
								"data" === i && !1 === /=/.test(s.peek())
									? ((o.inDataStep = !0), (o.nextword = !0), "builtin")
									: "proc" === i
										? ((o.inProc = !0), (o.nextword = !0), "builtin")
										: "%macro" === i
											? ((o.inMacro = !0), (o.nextword = !0), "builtin")
											: /title[1-9]/.test(i)
												? "def"
												: "footnote" === i
													? (s.eat(/[1-9]/), "def")
