function O(O) {
	for (var T = {}, E = O.split(" "), I = 0; I < E.length; ++I) T[E[I]] = !0;
	return T;
}
var T =
	...
	I = "BOOLEAN INT LONG FLOAT DOUBLE CHARARRAY BYTEARRAY BAG TUPLE MAP ",
	N = O(T),
	A = O(E),
	e = O(I),
	R = /[*+\-%<>=&?:\/!|]/;
...
function L(O, T) {
	...
	return '"' == I || "'" == I
		? S(
				...
				((E = I),
				function (O, T) {
					for (var I, N = !1, A = !1; null != (I = O.next()); ) {
						if (I == E && !N) {
							A = !0;
							break;
						}
				...
				: "/" == I
					...
					: "-" == I
						...
						: R.test(I)
							...
							: (O.eatWhile(/[\w\$_]/),
								A &&
								A.propertyIsEnumerable(O.current().toUpperCase()) &&
								!O.eat(")") &&
								!O.eat(".")
									? "keyword"
									: N && N.propertyIsEnumerable(O.current().toUpperCase())
										? "builtin"
										: e && e.propertyIsEnumerable(O.current().toUpperCase())
											? "type"
											: "variable");
}
const r = {
	...
	languageData: { autocomplete: (T + I + E).split(" ") },
};
export { r as pig };
