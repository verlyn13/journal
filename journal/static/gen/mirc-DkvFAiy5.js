...
var i = e(
	...
	r = e("if elseif else and not or eq ne in ni for foreach while switch"),
	t = /[+\-*&%=<>!?^\/\|]/;
function a(e, i, $) {
	return (i.tokenize = $), $(e, i);
}
function n(e, n) {
	var l = n.beforeParams;
	n.beforeParams = !1;
	var c = e.next();
	if (/[\[\]{}\(\),\.]/.test(c))
		return (
			"(" == c && l ? (n.inParams = !0) : ")" == c && (n.inParams = !1), null
		);
	if (/\d/.test(c)) return e.eatWhile(/[\w\.]/), "number";
	if ("\\" == c) return e.eat("\\"), e.eat(/./), "number";
	if ("/" == c && e.eat("*")) return a(e, n, o);
	if (";" == c && e.match(/ *\( *\(/)) return a(e, n, s);
	if (";" != c || n.inParams) {
		if ('"' == c) return e.eat(/"/), "keyword";
		if ("$" == c)
			return (
				e.eatWhile(/[$_a-z0-9A-Z\.:]/),
				i && i.propertyIsEnumerable(e.current().toLowerCase())
					? "keyword"
					: ((n.beforeParams = !0), "builtin")
			);
		if ("%" == c)
			return e.eatWhile(/[^,\s()]/), (n.beforeParams = !0), "string";
		if (t.test(c)) return e.eatWhile(t), "operator";
		e.eatWhile(/[\w\$_{}]/);
		var d = e.current().toLowerCase();
		return $ && $.propertyIsEnumerable(d)
			? "keyword"
...
const l = {
	name: "mirc",
	startState: function () {
		return { tokenize: n, beforeParams: !1, inParams: !1 };
	},
	token: function (e, i) {
