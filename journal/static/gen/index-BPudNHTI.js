			...
			if (37 == a) {
				...
				for (;;) {
					...
					if (32 == $ || 10 == $) ++a;
						...
						if (45 != $ || 2 != e) {
							let i = 101 == $ && 110 == O.peek(a + 1) && 100 == O.peek(a + 2);
							return void O.acceptToken(i ? 3 : 2, e);
						}
						e = ++a;
					}
				}
	...
	return new m((e) => {
		...
		for (;;) {
			...
			if (123 == $ && 37 == e.peek(1)) {
				...
				for (; ; $++) {
					let O = e.peek($);
					if (!(((r = O) >= 65 && r <= 90) || (r >= 97 && r <= 122))) break;
			...
			if ((e.advance(), 10 == $)) break;
		}
		var r;
		e.pos > i && e.acceptToken($);
	});
}
const q = p("endcomment", 182, 5),
	d = p("endraw", 181, 4),
	f = new m((O) => {
		if (35 == O.next) {
			for (
				O.advance();
				!(10 == O.next || O.next < 0) &&
				((37 != O.next && 125 != O.next) || 125 != O.peek(1));
			)
				O.advance();
			O.acceptToken(6);
		}
	...
	b = t.deserialize({
		...
		tokenizers: [P, d, q, f, 0, 1, 2, 3],
		topRules: { Template: [0, 7] },
		specialized: [
			{ term: 187, get: (O) => h[O] || -1 },
			{ term: 37, get: (O) => g[O] || -1 },
		],
		tokenPrec: 0,
	});
function v(O, $) {
	return O.split(" ").map((O) => ({ label: O, type: $ }));
}
const y = v(
	...
	k = v("empty forloop tablerowloop in with as contains", "keyword"),
	_ = v("first index index0 last length rindex", "property"),
	W = v(
		"col col0 col_first col_last first index index0 last length rindex rindex0 row",
		"property",
	);
function T(O = {}) {
	let $ = O.filters ? O.filters.concat(y) : y,
		a = O.tags ? O.tags.concat(u) : u,
		e = O.variables ? O.variables.concat(k) : k,
		{ properties: i } = O;
	return (O) => {
		var r;
		let t = (function (O) {
			var $;
			let { state: a, pos: e } = O,
				i = n(a).resolveInner(e, -1).enterUnfinishedNodesBefore(e),
				r =
					(null === ($ = i.childBefore(e)) || void 0 === $ ? void 0 : $.name) ||
					i.name;
			if ("FilterName" == i.name) return { type: "filter", node: i };
			if (O.explicit && "|" == r) return { type: "filter" };
			if ("TagName" == i.name) return { type: "tag", node: i };
			if (O.explicit && "{%" == r) return { type: "tag" };
			if ("PropertyName" == i.name && "MemberExpression" == i.parent.name)
				return { type: "property", node: i, target: i.parent };
			if ("." == i.name && "MemberExpression" == i.parent.name)
				return { type: "property", target: i.parent };
			if ("MemberExpression" == i.name && "." == r)
				return { type: "property", target: i };
			if ("VariableName" == i.name) return { type: "expression", from: i.from };
			let t = O.matchBefore(/[\w\u00c0-\uffff]+$/);
			return t
				? { type: "expression", from: t.from }
				: O.explicit &&
						"CommentText" != i.name &&
						"StringLiteral" != i.name &&
						"NumberLiteral" != i.name &&
						"InlineComment" != i.name
					? { type: "expression" }
					: null;
		})(O);
		if (!t) return null;
					...
					: "tag" == t.type
						...
						: "expression" == t.type
							...
							: (function (O, $, a, e) {
									let i = [];
									for (;;) {
										let a = $.getChild("Expression");
										if (!a) return [];
										if ("forloop" == a.name) return i.length ? [] : _;
										if ("tablerowloop" == a.name) return i.length ? [] : W;
										if ("VariableName" == a.name) {
											i.unshift(O.sliceDoc(a.from, a.to));
											break;
										}
										if ("MemberExpression" != a.name) return [];
										{
											let e = a.getChild("PropertyName");
											e && i.unshift(O.sliceDoc(e.from, e.to)), ($ = a);
										}
									}
									return e ? e(i, O, a) : [];
								})(O.state, t.target, O, i)),
			Q.length
				? { options: Q, from: c, validFor: /^[\w\u00c0-\uffff]*$/ }
				: null
		);
	};
}
const Y = O.inputHandler.of(
	(O, a, e, i) =>
		"%" == i &&
		a == e &&
		"{}" == O.state.doc.sliceString(a - 1, e + 1) &&
		(O.dispatch(
			O.state.changeByRange((O) => ({
				changes: { from: O.from, to: O.to, insert: "%%" },
				range: $.cursor(O.from + 1),
			})),
...
function R(O) {
	return ($) => {
		let a = O.test($.textAfter);
		return $.lineIndent($.node.from) + (a ? 0 : $.unit);
	};
}
const w = i.define({
		name: "liquid",
			...
			props: [
				...
				c.add({
					...
					"UnlessDirective ForDirective TablerowDirective CaptureDirective":
						R(/^\s*(\{%-?\s*)?end\w/),
					IfDirective: R(/^\s*(\{%-?\s*)?(endif|else|elsif)\b/),
					CaseDirective: R(/^\s*(\{%-?\s*)?(endcase|when)\b/),
				}),
				o.add({
					"UnlessDirective ForDirective TablerowDirective CaptureDirective IfDirective CaseDirective RawDirective Comment"(
						...
						let $ = O.firstChild,
							a = O.lastChild;
						return $ && "Tag" == $.name
							? { from: $.to, to: "EndTag" == a.name ? a.from : O.to }
							: null;
					},
				}),
			],
	...
	return w.configure(
		{
			wrap: r(($) =>
				$.type.isTop
					? {
							parser: O.parser,
							overlay: (O) => "Text" == O.name || "RawText" == O.name,
						}
					: null,
			),
		},
...
const j = X(U.language);
function G(O = {}) {
	let $ = O.base || U,
		a = $.language == U.language ? j : X($.language);
	return new e(a, [
		$.support,
		a.data.of({ autocomplete: T(O) }),
		$.language.data.of({ closeBrackets: { brackets: ["{"] } }),
		Y,
	]);
}
export {
	Y as closePercentBrace,
	G as liquid,
	T as liquidCompletionSource,
	j as liquidLanguage,
};
