function e(e) {
	...
	return {
		...
		token: function (c, s) {
			if ((c.column() || (s.context = 0), c.eatSpace())) return null;
			var o;
			if (c.eatWhile(/\w/)) {
				if (
					(e && c.eat(".") && c.eatWhile(/\w/),
					(o = c.current()),
					!c.indentation())
				)
					return c.match(a) ? "number" : null;
				if ((1 == s.context || 4 == s.context) && i.test(o))
					return (s.context = 4), "variable";
				if (2 == s.context && n.test(o))
					return (s.context = 4), "variableName.special";
				if (t.test(o)) return (s.context = 1), "keyword";
				if (r.test(o)) return (s.context = 2), "keyword";
				if (4 == s.context && a.test(o)) return "number";
				if (l.test(o)) return "error";
			} else {
				if (c.eat(";")) return c.skipToEnd(), "comment";
				if (c.eat('"')) {
					for (; (o = c.next()) && '"' != o; ) "\\" == o && c.next();
					return "string";
				}
				if (c.eat("'")) {
					if (c.match(/\\?.'/)) return "number";
				} else if (c.eat(".") || (c.sol() && c.eat("#"))) {
					if (((s.context = 5), c.eatWhile(/\w/))) return "def";
				} else if (c.eat("$")) {
					if (c.eatWhile(/[\da-f]/i)) return "number";
				} else if (c.eat("%")) {
					if (c.eatWhile(/[01]/)) return "number";
				} else c.next();
			}
			return null;
		},
