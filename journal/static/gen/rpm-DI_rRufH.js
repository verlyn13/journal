...
const a = {
	...
	token: function (a) {
		if (a.sol()) {
			if (a.match(r)) return "tag";
			if (a.match(e)) return "tag";
		}
		return a.match(t) ? "string" : (a.next(), null);
	},
};
var n =
...
const l = {
	...
	token: function (r, e) {
		...
		if (r.sol()) {
			if (r.match(c)) return "header";
			if (r.match(o)) return "atom";
		}
		if (r.match(/^\$\w+/)) return "def";
		if (r.match(/^\$\{\w+\}/)) return "def";
		if (r.match(p)) return "keyword";
		if (r.match(i)) return (e.controlFlow = !0), "keyword";
		if (e.controlFlow) {
			if (r.match(u)) return "operator";
			if (r.match(/^(\d+)/)) return "number";
			r.eol() && (e.controlFlow = !1);
		}
		if (r.match(n)) return r.eol() && (e.controlFlow = !1), "number";
		if (r.match(/^%[\w]+/))
...
export { a as rpmChanges, l as rpmSpec };
