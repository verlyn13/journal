...
const P = new $(
		(O, $) => {
			if ($.dialectEnabled(0)) {
				...
				else if (((e = O.peek(-1)) == d || e < 0) && $.canShift(160)) {
					let $ = 0;
					for (; O.next != d && S.includes(O.next); ) O.advance(), $++;
					O.next == d || y(O)
						? O.acceptToken(160, -$)
						: $ && O.acceptToken(162);
				...
				let $ = 0;
				for (; S.includes(O.next); ) O.advance(), $++;
				$ && O.acceptToken(162);
			}
		},
		{ contextual: !0 },
				...
				if ($ > -1) {
					...
					for (O.advance(); O.next >= 0; )
						if (O.next == d) {
							O.advance();
							let e = 0;
							for (; O.next != d && S.includes(O.next); ) e++, O.advance();
							if (e < $) {
								Q = -e - 1;
								break;
							}
	...
	k = new $((O, $) => {
		...
		let e = $.context.depth;
		if (O.next < 0 && e) O.acceptToken(155);
		else if (O.peek(-1) == d) {
			let $ = 0;
			for (; O.next != d && S.includes(O.next); ) O.advance(), $++;
			$ == e ||
				O.next == d ||
				y(O) ||
				($ < e ? O.acceptToken(155, -$) : O.acceptToken(154));
		}
	}),
	p = new $((O, $) => {
		for (let e = !1, Q = 0, a = 0; ; a++) {
			let { next: t } = O;
			if (!(c(t) || 45 == t || 95 == t || (e && X(t)))) {
				if (35 == t && 123 == O.peek(1)) {
					O.acceptToken(5, 2);
					break;
				}
				e && O.acceptToken(40 == t ? 157 : 2 == Q && $.canShift(4) ? 4 : 158);
				break;
			}
			!e && (45 != t || a > 0) && (e = !0),
				Q === a && 45 == t && Q++,
	...
	u = new $((O) => {
		if (125 == O.next) {
			for (
				O.advance();
				c(O.next) || 45 == O.next || 95 == O.next || X(O.next);
			)
				O.advance();
...
const T = new Q({
	...
	Y = O.deserialize({
		...
		tokenizers: [k, Z, u, f, p, P, s, m, 0, 1, 2, 3],
		topRules: { StyleSheet: [0, 10], Styles: [1, 134] },
		dialects: { indented: 0 },
		specialized: [
			{ term: 158, get: (O) => j[O] || -1 },
			{ term: 157, get: (O) => U[O] || -1 },
			{ term: 77, get: (O) => z[O] || -1 },
		],
		tokenPrec: 3003,
	}),
	...
	x = h.configure({
		...
		props: [
			...
			R.add({ Block: (O) => ({ from: O.from, to: O.to }) }),
		],
	}),
	G = t((O) => "VariableName" == O.name || "SassVariableName" == O.name);
function g(O) {
	return new i(
		(null == O ? void 0 : O.indented) ? x : h,
		h.data.of({ autocomplete: G }),
	);
}
export { g as sass, G as sassCompletionSource, h as sassLanguage };
