...
var Dt = gt("__proto__,__v_isRef,__isVue"),
	Yt = new Set(
				...
				if (1 == c.nodeType) {
					...
					if (
						...
						if (f) h.scrollBy(p, m);
							...
							if (m) {
								let e = c.scrollTop;
								(c.scrollTop += m / O), (i = (c.scrollTop - e) * O);
							}
							if (p) {
								let t = c.scrollLeft;
...
function ga(e, t, i) {
	...
	for (let f = e.firstChild; f; f = f.nextSibling) {
		...
		for (let d = 0; d < e.length; d++) {
			...
			if (!n || o > m || (o == m && s > p)) {
				(n = f), (r = O), (s = p), (o = m);
				let a = m ? (i < O.top ? -1 : 1) : p ? (t < O.left ? -1 : 1) : 0;
...
const rd = [
	...
	sd = [
		...
		{ key: "ArrowRight", run: gf, shift: Af, preventDefault: !0 },
		{
			key: "Mod-ArrowRight",
			mac: "Alt-ArrowRight",
			run: (e) => xf(e, pf(e)),
...
function CO(e, t) {
	...
	for (let e of t) {
		...
		if (n < i.length && i[n].from < e.from) {
			let t = i[n];
			t instanceof gO &&
				(i[n] = new gO(t.type, t.from, t.to, CO(t.children, [e])));
		} else i.splice(n++, 0, e);
	}
	return i;
}
...
function rm(e, t, i) {
	let n;
	for (let r of i)
		if (!r.attrs || r.attrs(n || (n = im(e.node.parent.firstChild, t))))
...
function sm(e = [], t = []) {
	...
	return Wh((e, t) => {
		let a = e.type.id;
		if (28 == a) return rm(e, t, i);
		if (31 == a) return rm(e, t, n);
		...
		if (20 == a && s.length) {
			...
			if (o)
				for (let e of s)
					if (e.tag == o && (!e.attrs || e.attrs(i || (i = im(r, t))))) {
						let t = n.lastChild,
		...
		if (o && 23 == a) {
			...
			if ((i = n.firstChild)) {
				...
				if (e)
					for (let i of e) {
						if (i.tagName && i.tagName != nm(n.parent, t)) continue;
						let e = n.lastChild;
						if (26 == e.type.id) {
							let t = e.from + 1,
								n = e.lastChild,
...
const Tm = (e) => (t) => {
		...
		if (
			...
			return { from: r.from, options: xm(), validFor: wm };
		if ("ValueName" == r.name)
			return { from: r.from, options: Sm, validFor: wm };
		if ("PseudoClassName" == r.name)
			return { from: r.from, options: bm, validFor: wm };
		if (
			e(r) ||
			((t.explicit || s) &&
				(function (e, t) {
		...
		if ("TagName" == r.name) {
			for (let { parent: e } = r; e; e = e.parent)
				if ("Block" == e.name)
					return { from: r.from, options: xm(), validFor: wm };
			return { from: r.from, options: ym, validFor: wm };
		}
		if ("AtKeyword" == r.name)
			return { from: r.from, options: Qm, validFor: wm };
		if (!t.explicit) return null;
		let o = r.resolve(n),
			a = o.childBefore(n);
		return a && ":" == a.name && "PseudoClassSelector" == o.name
			? { from: n, options: bm, validFor: wm }
			: (a && ":" == a.name && "Declaration" == o.name) || "ArgList" == o.name
				? { from: n, options: Sm, validFor: wm }
				: "Block" == o.name || "Styles" == o.name
					? { from: n, options: xm(), validFor: wm }
					: null;
	},
...
const Wm = new gp((e, t) => {
		...
		let i = 0;
		for (; Rm.indexOf(e.next) > -1; ) e.advance(), i++;
		if (Lm(e.next, !0)) {
			for (e.advance(), i++; Lm(e.next, !1); ) e.advance(), i++;
			for (; Rm.indexOf(e.next) > -1; ) e.advance(), i++;
			if (44 == e.next) return;
			for (let t = 0; ; t++) {
				if (7 == t) {
	...
	return (
		t.cursor(xh.IncludeAnonymous).iterate((t) => {
			if (r) r = !1;
				...
				for (let i of eg(e, t.node)) n.push(i);
				return !1;
			}
		}),
		Nm.set(t, n),
...
function ng(e) {
	...
	if (ig.indexOf(t.name) > -1) return null;
	let i =
		"VariableName" == t.name ||
	...
	for (let i = t; i; i = i.parent)
		Hm.has(i.name) && (n = n.concat(eg(e.state.doc, i)));
	return { options: n, from: i ? t.from : e.pos, validFor: tg };
}
function rg(e, t, i) {
	var n;
	let r = [];
...
function sg(e) {
	...
	return "PropertyName" == i.name
		...
		: ("." != i.name && "?." != i.name) || "MemberExpression" != i.parent.name
			? ig.indexOf(i.name) > -1
				? null
				: "VariableName" == i.name || (i.to - i.from < 20 && tg.test(t(i)))
					? { path: [], name: t(i) }
...
function Og(e = {}) {
	let t = e.jsx ? (e.typescript ? cg : hg) : e.typescript ? lg : og,
		i = e.typescript ? Gm.concat(dg) : Um.concat(fg);
	return new Wc(t, [
		og.data.of({ autocomplete: cd(ig, hd(i)) }),
		og.data.of({ autocomplete: ng }),
		e.jsx ? gg : [],
	]);
}
function pg(e, t, i = e.length) {
...
const mg =
	...
	gg = Gl.inputHandler.of((e, t, i, n, r) => {
		if (
...
function bg(e, t, i) {
	let n = xg(e.line, e.column, t, i),
		r = {
			from: n,
			to:
				null != e.endLine && 1 != e.endColumn
					? xg(e.endLine, e.endColumn, t, i)
					: n,
			message: e.message,
	...
	esLint: function (e, t) {
		return (
			...
			(i) => {
				let { state: n } = i,
					r = [];
				for (let { from: i, to: s } of og.findRegions(n)) {
					let o = n.doc.lineAt(i),
						a = { line: o.number - 1, col: i - o.from, pos: i };
					for (let o of e.verify(n.sliceDoc(i, s), t)) r.push(bg(o, n.doc, a));
				}
				return r;
			}
		);
	...
	scopeCompletionSource: function (e) {
		...
		return (i) => {
			...
			return (
				...
				{ from: i.pos - n.name.length, options: s, validFor: tg }
			);
		};
	},
	snippets: Um,
...
function Cg(e, t, i) {
	let n = i.tags[Xg(e, Ag(t))];
	return (null == n ? void 0 : n.children) || i.allTags;
}
function Rg(e, t) {
	let i = [];
	for (let n = Ag(t); n && !n.type.isTop; n = Ag(n.parent)) {
		let r = Xg(e, n);
					...
					: "Is" == r.name ||
						...
						: !t.explicit ||
							...
							: (function (e, t, i, n) {
									let r = [],
										s = 0;
									for (let n of Cg(e.doc, i, t))
										r.push({ label: "<" + n, type: "type" });
									for (let t of Rg(e.doc, i))
										r.push({
											label: "</" + t + ">",
											type: "type",
											boost: 99 - s++,
	...
	!1 === e.matchClosingTags && (i = "noMatch"),
		...
		((e.nestedLanguages && e.nestedLanguages.length) ||
			...
			(t = sm(
				...
				(e.nestedAttributes || []).concat(Dg),
			));
	let n = t
		? Yg.configure({ wrap: t, dialect: i })
		: i
			? Bg.configure({ dialect: i })
			: Bg;
	return new Wc(n, [
...
function ex(e, t) {
	...
	for (;;) {
		let e,
			n = i.nextSibling;
		if (!n || (null != (e = Jg(n.type)) && e <= t)) break;
		i = n;
	}
	return i.to;
}
...
function Qx(e) {
	return import("./index-Dq_2wXEr.js").then((t) => t.sql({ dialect: t[e] }));
}
const wx = [
		zc.of({
			name: "C",
			extensions: ["c", "h", "ino"],
			load: () => import("./index-BdIA5_QL.js").then((e) => e.cpp()),
		}),
		zc.of({
			name: "C++",
			alias: ["cpp"],
		...
		zc.of({
			name: "Go",
			extensions: ["go"],
			load: () => import("./index-DuCMWz6o.js").then((e) => e.go()),
		}),
		zc.of({
			name: "HTML",
			alias: ["xhtml"],
		...
		zc.of({
			name: "Java",
			extensions: ["java"],
			load: () => import("./index-CTeLx_nc.js").then((e) => e.java()),
		}),
		zc.of({
			name: "JavaScript",
		...
		zc.of({
			...
			alias: ["json5"],
			extensions: ["json", "map"],
			load: () => import("./index-B04t5VcT.js").then((e) => e.json()),
		}),
		zc.of({
			name: "JSX",
			extensions: ["jsx"],
		...
		zc.of({
			name: "LESS",
			extensions: ["less"],
			load: () => import("./index-CtbAeB2w.js").then((e) => e.less()),
		}),
		zc.of({
			name: "Liquid",
			extensions: ["liquid"],
			load: () => import("./index-Bf1Fn9qy.js").then((e) => e.liquid()),
		}),
		...
		zc.of({
			...
			load: () => import("./index-Cu31-hBR.js").then((e) => e.php()),
		}),
		...
		zc.of({
			...
			filename: /^(BUCK|BUILD)$/,
			load: () => import("./index-D1e8LekQ.js").then((e) => e.python()),
		}),
		zc.of({
			name: "Rust",
			extensions: ["rs"],
			load: () => import("./index-txOjinq8.js").then((e) => e.rust()),
		}),
		zc.of({
			name: "Sass",
			extensions: ["sass"],
			load: () =>
				import("./index-B_VdopL8.js").then((e) => e.sass({ indented: !0 })),
		}),
		zc.of({
			name: "SCSS",
			extensions: ["scss"],
			load: () => import("./index-B_VdopL8.js").then((e) => e.sass()),
		}),
		...
		zc.of({
			name: "WebAssembly",
			extensions: ["wat", "wast"],
			load: () => import("./index-CGjARpTv.js").then((e) => e.wast()),
		}),
		zc.of({
			name: "XML",
			alias: ["rss", "wsdl", "xsd"],
			extensions: ["xml", "xsl", "xsd", "svg"],
			load: () => import("./index-C61fujCY.js").then((e) => e.xml()),
		}),
		zc.of({
			name: "YAML",
			alias: ["yml"],
			extensions: ["yaml", "yml"],
			load: () => import("./index-tzNGxWq3.js").then((e) => e.yaml()),
		}),
		zc.of({
			name: "APL",
			extensions: ["dyalog", "apl"],
			load: () => import("./apl-CXdQSnD-.js").then((e) => yx(e.apl)),
		}),
		zc.of({
			name: "PGP",
			alias: ["asciiarmor"],
			...
			load: () =>
				import("./asciiarmor-D5V0T9Cu.js").then((e) => yx(e.asciiArmor)),
		}),
		zc.of({
			name: "ASN.1",
			extensions: ["asn", "asn1"],
			load: () => import("./asn1-DWPaVWf6.js").then((e) => yx(e.asn1({}))),
		}),
		zc.of({
			name: "Asterisk",
			filename: /^extensions\.conf$/i,
			load: () => import("./asterisk-B46aRdXb.js").then((e) => yx(e.asterisk)),
		}),
		zc.of({
			name: "Brainfuck",
			extensions: ["b", "bf"],
			load: () =>
				import("./brainfuck-CtZaYBKt.js").then((e) => yx(e.brainfuck)),
		}),
		zc.of({
			name: "Cobol",
			extensions: ["cob", "cpy"],
			load: () => import("./cobol-stawOqaz.js").then((e) => yx(e.cobol)),
		}),
		zc.of({
			name: "C#",
			alias: ["csharp", "cs"],
			extensions: ["cs"],
			load: () => import("./clike-BgBwzOhw.js").then((e) => yx(e.csharp)),
		}),
		zc.of({
			name: "Clojure",
			extensions: ["clj", "cljc", "cljx"],
			load: () => import("./clojure-DR_hEDJv.js").then((e) => yx(e.clojure)),
		}),
		zc.of({
			name: "ClojureScript",
			extensions: ["cljs"],
		...
		zc.of({
			...
			extensions: ["gss"],
			load: () => import("./css-ZjFoif2m.js").then((e) => yx(e.gss)),
		}),
		zc.of({
			name: "CMake",
			extensions: ["cmake", "cmake.in"],
			filename: /^CMakeLists\.txt$/,
			load: () => import("./cmake-DXn5xaP-.js").then((e) => yx(e.cmake)),
		}),
		zc.of({
			name: "CoffeeScript",
			...
			extensions: ["coffee"],
			load: () =>
				import("./coffeescript-BTDxI-eT.js").then((e) => yx(e.coffeeScript)),
		}),
		zc.of({
			name: "Common Lisp",
			alias: ["lisp"],
			...
			load: () =>
				import("./commonlisp-vq6DHwts.js").then((e) => yx(e.commonLisp)),
		}),
		zc.of({
			name: "Cypher",
			extensions: ["cyp", "cypher"],
			load: () => import("./cypher-gJDei3ni.js").then((e) => yx(e.cypher)),
		}),
		zc.of({
			name: "Cython",
			extensions: ["pyx", "pxd", "pxi"],
			load: () => import("./python-DD-Q2bae.js").then((e) => yx(e.cython)),
		}),
		zc.of({
			name: "Crystal",
			extensions: ["cr"],
			load: () => import("./crystal-CRzZ78HM.js").then((e) => yx(e.crystal)),
		}),
		zc.of({
			name: "D",
			extensions: ["d"],
			load: () => import("./d-BShv7fp-.js").then((e) => yx(e.d)),
		}),
		zc.of({
			name: "Dart",
			extensions: ["dart"],
			load: () => import("./clike-BgBwzOhw.js").then((e) => yx(e.dart)),
		}),
		zc.of({
			name: "diff",
			extensions: ["diff", "patch"],
			load: () => import("./diff-UJJ5BM9S.js").then((e) => yx(e.diff)),
		}),
		zc.of({
			name: "Dockerfile",
			filename: /^Dockerfile$/,
			load: () =>
				import("./dockerfile-knGOOxt4.js").then((e) => yx(e.dockerFile)),
		}),
		zc.of({
			name: "DTD",
			extensions: ["dtd"],
			load: () => import("./dtd-DI9Herc-.js").then((e) => yx(e.dtd)),
		}),
		zc.of({
			name: "Dylan",
			extensions: ["dylan", "dyl", "intr"],
			load: () => import("./dylan-CHJ5qNM_.js").then((e) => yx(e.dylan)),
		}),
		zc.of({
			name: "EBNF",
			load: () => import("./ebnf-B-Df8AvO.js").then((e) => yx(e.ebnf)),
		}),
		zc.of({
			name: "ECL",
			extensions: ["ecl"],
			load: () => import("./ecl-DPFc0Uaf.js").then((e) => yx(e.ecl)),
		}),
		zc.of({
			name: "edn",
			extensions: ["edn"],
		...
		zc.of({
			name: "Eiffel",
			extensions: ["e"],
			load: () => import("./eiffel-DR5c0IrL.js").then((e) => yx(e.eiffel)),
		}),
		zc.of({
			name: "Elm",
			extensions: ["elm"],
			load: () => import("./elm-D7xaZ7Da.js").then((e) => yx(e.elm)),
		}),
		zc.of({
			name: "Erlang",
			extensions: ["erl"],
			load: () => import("./erlang-BuqgqA5h.js").then((e) => yx(e.erlang)),
		}),
		zc.of({
			name: "Esper",
			load: () => import("./sql-B3SPsWL6.js").then((e) => yx(e.esper)),
		}),
		zc.of({
			name: "Factor",
			extensions: ["factor"],
			load: () => import("./factor-CFXhaUWb.js").then((e) => yx(e.factor)),
		}),
		zc.of({
			name: "FCL",
			load: () => import("./fcl-CVaBTCzQ.js").then((e) => yx(e.fcl)),
		}),
		zc.of({
			name: "Forth",
			extensions: ["forth", "fth", "4th"],
			load: () => import("./forth-Dri_5nFc.js").then((e) => yx(e.forth)),
		}),
		zc.of({
			name: "Fortran",
			extensions: ["f", "for", "f77", "f90", "f95"],
			load: () => import("./fortran-DL7KluxX.js").then((e) => yx(e.fortran)),
		}),
		zc.of({
			name: "F#",
			alias: ["fsharp"],
			extensions: ["fs"],
			load: () => import("./mllike-3c2dg9RT.js").then((e) => yx(e.fSharp)),
		}),
		zc.of({
			name: "Gas",
			extensions: ["s"],
			load: () => import("./gas-5kEe4nk0.js").then((e) => yx(e.gas)),
		}),
		zc.of({
			name: "Gherkin",
			extensions: ["feature"],
			load: () => import("./gherkin-B6X1vGSW.js").then((e) => yx(e.gherkin)),
		}),
		zc.of({
			name: "Groovy",
			extensions: ["groovy", "gradle"],
			filename: /^Jenkinsfile$/,
			load: () => import("./groovy-DnIzCrpZ.js").then((e) => yx(e.groovy)),
		}),
		zc.of({
			name: "Haskell",
			extensions: ["hs"],
			load: () => import("./haskell-CmdsSjFB.js").then((e) => yx(e.haskell)),
		}),
		zc.of({
			name: "Haxe",
			extensions: ["hx"],
			load: () => import("./haxe-D25e-2l-.js").then((e) => yx(e.haxe)),
		}),
		zc.of({
			name: "HXML",
			extensions: ["hxml"],
			load: () => import("./haxe-D25e-2l-.js").then((e) => yx(e.hxml)),
		}),
		zc.of({
			name: "HTTP",
			load: () => import("./http-BKJkqRj4.js").then((e) => yx(e.http)),
		}),
		zc.of({
			name: "IDL",
			extensions: ["pro"],
			load: () => import("./idl-pcmmZuaN.js").then((e) => yx(e.idl)),
		}),
		zc.of({
			name: "JSON-LD",
			alias: ["jsonld"],
			extensions: ["jsonld"],
			load: () => import("./javascript-D48wDJXV.js").then((e) => yx(e.jsonld)),
		}),
		zc.of({
			name: "Jinja2",
			extensions: ["j2", "jinja", "jinja2"],
			load: () => import("./jinja2-DhgKlfW7.js").then((e) => yx(e.jinja2)),
		}),
		zc.of({
			name: "Julia",
			extensions: ["jl"],
			load: () => import("./julia-DjdwvFuI.js").then((e) => yx(e.julia)),
		}),
		zc.of({
			name: "Kotlin",
			extensions: ["kt", "kts"],
			load: () => import("./clike-BgBwzOhw.js").then((e) => yx(e.kotlin)),
		}),
		zc.of({
			name: "LiveScript",
			alias: ["ls"],
			extensions: ["ls"],
			load: () =>
				import("./livescript-HO6ttBFx.js").then((e) => yx(e.liveScript)),
		}),
		zc.of({
			name: "Lua",
			extensions: ["lua"],
			load: () => import("./lua-8DXOpKDz.js").then((e) => yx(e.lua)),
		}),
		zc.of({
			name: "mIRC",
			extensions: ["mrc"],
			load: () => import("./mirc-DkvFAiy5.js").then((e) => yx(e.mirc)),
		}),
		zc.of({
			name: "Mathematica",
			extensions: ["m", "nb", "wl", "wls"],
			load: () =>
				import("./mathematica-IIbmv7SK.js").then((e) => yx(e.mathematica)),
		}),
		zc.of({
			name: "Modelica",
			extensions: ["mo"],
			load: () => import("./modelica-DSrSSXCg.js").then((e) => yx(e.modelica)),
		}),
		zc.of({
			name: "MUMPS",
			extensions: ["mps"],
			load: () => import("./mumps-zvxmGV4l.js").then((e) => yx(e.mumps)),
		}),
		zc.of({
			name: "Mbox",
			extensions: ["mbox"],
			load: () => import("./mbox-z_N2Mo3d.js").then((e) => yx(e.mbox)),
		}),
		zc.of({
			name: "Nginx",
			filename: /nginx.*\.conf$/i,
			load: () => import("./nginx-B4hAVlXN.js").then((e) => yx(e.nginx)),
		}),
		zc.of({
			name: "NSIS",
			extensions: ["nsh", "nsi"],
			load: () => import("./nsis-BvKfQ9lQ.js").then((e) => yx(e.nsis)),
		}),
		zc.of({
			name: "NTriples",
			extensions: ["nt", "nq"],
			load: () => import("./ntriples-FFvkTduk.js").then((e) => yx(e.ntriples)),
		}),
		zc.of({
			name: "Objective-C",
			alias: ["objective-c", "objc"],
			extensions: ["m"],
			load: () => import("./clike-BgBwzOhw.js").then((e) => yx(e.objectiveC)),
		}),
		zc.of({
			name: "Objective-C++",
			alias: ["objective-c++", "objc++"],
			extensions: ["mm"],
			load: () => import("./clike-BgBwzOhw.js").then((e) => yx(e.objectiveCpp)),
		}),
		zc.of({
			name: "OCaml",
			extensions: ["ml", "mli", "mll", "mly"],
			load: () => import("./mllike-3c2dg9RT.js").then((e) => yx(e.oCaml)),
		}),
		zc.of({
			name: "Octave",
			extensions: ["m"],
			load: () => import("./octave-C4hdQ5Hf.js").then((e) => yx(e.octave)),
		}),
		zc.of({
			name: "Oz",
			extensions: ["oz"],
			load: () => import("./oz-qQq7irPq.js").then((e) => yx(e.oz)),
		}),
		zc.of({
			name: "Pascal",
			extensions: ["p", "pas"],
			load: () => import("./pascal-CafrKXZF.js").then((e) => yx(e.pascal)),
		}),
		zc.of({
			name: "Perl",
			extensions: ["pl", "pm"],
			load: () => import("./perl-tdMlh9QA.js").then((e) => yx(e.perl)),
		}),
		zc.of({
			name: "Pig",
			extensions: ["pig"],
			load: () => import("./pig-CwGJVRC4.js").then((e) => yx(e.pig)),
		}),
		zc.of({
			name: "PowerShell",
			extensions: ["ps1", "psd1", "psm1"],
			load: () =>
				import("./powershell-D1ly3AHC.js").then((e) => yx(e.powerShell)),
		}),
		zc.of({
			...
			load: () =>
				import("./properties-_nPj978X.js").then((e) => yx(e.properties)),
		}),
		zc.of({
			name: "ProtoBuf",
			extensions: ["proto"],
			load: () => import("./protobuf-B4y9do6W.js").then((e) => yx(e.protobuf)),
		}),
		zc.of({
			name: "Pug",
			alias: ["jade"],
			extensions: ["pug", "jade"],
			load: () => import("./pug-CMX2P1Iw.js").then((e) => yx(e.pug)),
		}),
		zc.of({
			name: "Puppet",
			extensions: ["pp"],
			load: () => import("./puppet-Bc-yJKzi.js").then((e) => yx(e.puppet)),
		}),
		zc.of({
			name: "Q",
			extensions: ["q"],
			load: () => import("./q-B0kTJ4cI.js").then((e) => yx(e.q)),
		}),
		zc.of({
			name: "R",
			alias: ["rscript"],
			extensions: ["r", "R"],
			load: () => import("./r-BA4EbLKE.js").then((e) => yx(e.r)),
		}),
		zc.of({
			name: "RPM Changes",
			load: () => import("./rpm-DI_rRufH.js").then((e) => yx(e.rpmChanges)),
		}),
		zc.of({
			name: "RPM Spec",
			extensions: ["spec"],
			load: () => import("./rpm-DI_rRufH.js").then((e) => yx(e.rpmSpec)),
		}),
		zc.of({
			name: "Ruby",
			...
			load: () => import("./ruby-CR-u2WLS.js").then((e) => yx(e.ruby)),
		}),
		zc.of({
			name: "SAS",
			extensions: ["sas"],
			load: () => import("./sas-Bqaci40L.js").then((e) => yx(e.sas)),
		}),
		zc.of({
			name: "Scala",
			extensions: ["scala"],
			load: () => import("./clike-BgBwzOhw.js").then((e) => yx(e.scala)),
		}),
		zc.of({
			name: "Scheme",
			extensions: ["scm", "ss"],
			load: () => import("./scheme-qjP0V5Ag.js").then((e) => yx(e.scheme)),
		}),
		zc.of({
			name: "Shell",
			alias: ["bash", "sh", "zsh"],
			...
			filename: /^PKGBUILD$/,
			load: () => import("./shell-BLav31RL.js").then((e) => yx(e.shell)),
		}),
		zc.of({
			name: "Sieve",
			extensions: ["siv", "sieve"],
			load: () => import("./sieve-r025o_jW.js").then((e) => yx(e.sieve)),
		}),
		zc.of({
			name: "Smalltalk",
			extensions: ["st"],
			load: () =>
				import("./smalltalk-C7J3IyEP.js").then((e) => yx(e.smalltalk)),
		}),
		zc.of({
			name: "Solr",
			load: () => import("./solr-78MBEUkU.js").then((e) => yx(e.solr)),
		}),
		zc.of({
			name: "SML",
			extensions: ["sml", "sig", "fun", "smackspec"],
			load: () => import("./mllike-3c2dg9RT.js").then((e) => yx(e.sml)),
		}),
		zc.of({
			name: "SPARQL",
			alias: ["sparul"],
			extensions: ["rq", "sparql"],
			load: () => import("./sparql-BRLJFz69.js").then((e) => yx(e.sparql)),
		}),
		zc.of({
			name: "Spreadsheet",
			alias: ["excel", "formula"],
			load: () =>
				import("./spreadsheet-CGlWr0eM.js").then((e) => yx(e.spreadsheet)),
		}),
		zc.of({
			name: "Squirrel",
			extensions: ["nut"],
			load: () => import("./clike-BgBwzOhw.js").then((e) => yx(e.squirrel)),
		}),
		zc.of({
			name: "Stylus",
			extensions: ["styl"],
			load: () => import("./stylus-3mOX9C7I.js").then((e) => yx(e.stylus)),
		}),
		zc.of({
			name: "Swift",
			extensions: ["swift"],
			load: () => import("./swift-B_XXVlXX.js").then((e) => yx(e.swift)),
		}),
		zc.of({
			name: "sTeX",
			load: () => import("./stex-Ba7Sj9VR.js").then((e) => yx(e.stex)),
		}),
		zc.of({
			name: "LaTeX",
			alias: ["tex"],
		...
		zc.of({
			name: "SystemVerilog",
			extensions: ["v", "sv", "svh"],
			load: () => import("./verilog-B4LBqg_1.js").then((e) => yx(e.verilog)),
		}),
		zc.of({
			name: "Tcl",
			extensions: ["tcl"],
			load: () => import("./tcl-B2fODqDH.js").then((e) => yx(e.tcl)),
		}),
		zc.of({
			name: "Textile",
			extensions: ["textile"],
			load: () => import("./textile-BuVhTOzU.js").then((e) => yx(e.textile)),
		}),
		zc.of({
			name: "TiddlyWiki",
			load: () =>
				import("./tiddlywiki-BgR6AEZu.js").then((e) => yx(e.tiddlyWiki)),
		}),
		zc.of({
			name: "Tiki wiki",
			load: () => import("./tiki-CI-ztowC.js").then((e) => yx(e.tiki)),
		}),
		zc.of({
			name: "TOML",
			extensions: ["toml"],
			load: () => import("./toml-BQXy8elZ.js").then((e) => yx(e.toml)),
		}),
		zc.of({
			name: "Troff",
			extensions: ["1", "2", "3", "4", "5", "6", "7", "8", "9"],
			load: () => import("./troff-Ce85hlLp.js").then((e) => yx(e.troff)),
		}),
		zc.of({
			name: "TTCN",
			extensions: ["ttcn", "ttcn3", "ttcnpp"],
			load: () => import("./ttcn-BxnYHtq8.js").then((e) => yx(e.ttcn)),
		}),
		zc.of({
			name: "TTCN_CFG",
			extensions: ["cfg"],
			load: () => import("./ttcn-cfg-CKJxb-l2.js").then((e) => yx(e.ttcnCfg)),
		}),
		zc.of({
			name: "Turtle",
			extensions: ["ttl"],
			load: () => import("./turtle-BkSuIpt-.js").then((e) => yx(e.turtle)),
		}),
		zc.of({
			name: "Web IDL",
			extensions: ["webidl"],
			load: () => import("./webidl-CGwyPRYs.js").then((e) => yx(e.webIDL)),
		}),
		zc.of({
			name: "VB.NET",
			extensions: ["vb"],
			load: () => import("./vb-BHgjm0DN.js").then((e) => yx(e.vb)),
		}),
		zc.of({
			name: "VBScript",
			extensions: ["vbs"],
			load: () => import("./vbscript-Dawdd5GZ.js").then((e) => yx(e.vbScript)),
		}),
		zc.of({
			name: "Velocity",
			extensions: ["vtl"],
			load: () => import("./velocity-BdD_26Jv.js").then((e) => yx(e.velocity)),
		}),
		zc.of({
			name: "Verilog",
			extensions: ["v"],
		...
		zc.of({
			name: "VHDL",
			extensions: ["vhd", "vhdl"],
			load: () => import("./vhdl-CWWH2SUo.js").then((e) => yx(e.vhdl)),
		}),
		zc.of({
			name: "XQuery",
			extensions: ["xy", "xquery"],
			load: () => import("./xquery-CVDBqZPY.js").then((e) => yx(e.xQuery)),
		}),
		zc.of({
			name: "Yacas",
			extensions: ["ys"],
			load: () => import("./yacas-I8QjZlxf.js").then((e) => yx(e.yacas)),
		}),
		zc.of({
			name: "Z80",
			extensions: ["z80"],
			load: () => import("./z80-D8p3gKXp.js").then((e) => yx(e.z80)),
		}),
		zc.of({
			name: "MscGen",
			extensions: ["mscgen", "mscin", "msc"],
			load: () => import("./mscgen-B2igx18G.js").then((e) => yx(e.mscgen)),
		}),
		zc.of({
			name: "Xù",
			extensions: ["xu"],
			load: () => import("./mscgen-B2igx18G.js").then((e) => yx(e.xu)),
		}),
		zc.of({
			name: "MsGenny",
			extensions: ["msgenny"],
			load: () => import("./mscgen-B2igx18G.js").then((e) => yx(e.msgenny)),
		}),
		zc.of({
			name: "Vue",
			extensions: ["vue"],
			load: () => import("./index-DQuu-65i.js").then((e) => e.vue()),
		}),
		zc.of({
			name: "Angular Template",
			load: () => import("./index-B4uqoX9d.js").then((e) => e.angular()),
		}),
	],
