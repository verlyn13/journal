...
const Y = new Q(
	(O, Q) => {
		for (
			let e = 0, a = O.next;
			((Q.context &&
				(a < 0 || 10 == a || 13 == a || (47 == a && 47 == O.peek(e + 1)))) ||
				41 == a ||
				125 == a) &&
				O.acceptToken(177),
				32 == a || 9 == a;
...
const x = new t({ start: !1, shift: (O, Q) => (179 == Q ? O : R.has(Q)) }),
	w = a({
		"func interface struct chan map const type var": X.definitionKeyword,
		"import package": X.moduleKeyword,
		...
		DefName: X.definition(X.variableName),
		TypeName: X.typeName,
		LabelName: X.labelName,
		FieldName: X.propertyName,
		"FunctionDecl/DefName": X.function(X.definition(X.variableName)),
		"TypeSpec/DefName": X.definition(X.typeName),
		"CallExpr/VariableName": X.function(X.variableName),
		...
		"DerefOp .": X.derefOperator,
		"UpdateOp IncDecOp": X.updateOperator,
		CompareOp: X.compareOperator,
		"= :=": X.definitionOperator,
		"<-": X.operator,
		'~ "*"': X.modifier,
		"; ,": X.separator,
		"... :": X.punctuation,
	...
	k = O.deserialize({
		...
		tokenizers: [Y, 1, 2, new e("j~RQYZXz{^~^O$|~~aP!P!Qd~iO$}~~", 25, 181)],
		topRules: { SourceFile: [0, 3] },
		dynamicPrecedences: { 19: 1, 51: -1, 55: 2, 69: -1, 108: -1 },
		specialized: [{ term: 184, get: (O) => s[O] || -1 }],
		tokenPrec: 5451,
	}),
	d = [
		i("func ${name}(${params}) ${type} {\n\t${}\n}", {
			label: "func",
			detail: "declaration",
			type: "keyword",
		}),
		i("func (${receiver}) ${name}(${params}) ${type} {\n\t${}\n}", {
			label: "func",
...
function u(O, Q) {
	return (e, a) => {
		O: for (let t = e.node.firstChild, X = 0, P = null; ; ) {
			for (; !t; ) {
				if (!X) break O;
				X--, (t = P.nextSibling), (P = P.parent);
			}
			(Q && t.name == Q) || "SpecList" == t.name
				? (X++, (P = t), (t = t.firstChild))
				: ("DefName" == t.name && a(t, O), (t = t.nextSibling));
		}
		return !0;
	};
}
const f = {
	FunctionDecl: u("function"),
	VarDecl: u("var", "VarSpec"),
	ConstDecl: u("constant", "ConstSpec"),
	TypeDecl: u("type", "TypeSpec"),
	ImportDecl: u("constant", "ImportSpec"),
	Parameter: u("var"),
	__proto__: null,
};
function j(O, Q) {
	...
	let a = [],
		t = !0;
	function X(Q, e) {
		let t = O.sliceString(Q.from, Q.to);
		a.push({ label: t, type: e });
	}
	return (
		Q.cursor(m.IncludeAnonymous).iterate((Q) => {
			if (t) t = !1;
			else if (Q.name) {
				let O = f[Q.name];
				if ((O && O(Q, X)) || y.has(Q.name)) return !1;
			} else if (Q.to - Q.from > 8192) {
				for (let e of j(O, Q.node)) a.push(e);
				return !1;
			}
		}),
		h.set(Q, a),
...
const g = /^[\w$\xa1-\uffff][\w$\d\xa1-\uffff]*$/,
	...
	G = (O) => {
		...
		for (let e = Q; e; e = e.parent)
			y.has(e.name) && (a = a.concat(j(O.state.doc, e)));
		return { options: a, from: e ? Q.from : O.pos, validFor: g };
	},
	b = P.define({
		name: "go",
		parser: k.configure({
			props: [
				$.add({
					IfStatement: r({ except: /^\s*({|else\b)/ }),
					LabeledStatement: T,
					"SwitchBlock SelectBlock": (O) => {
						let Q = O.textAfter,
							e = /^\s*\}/.test(Q),
							a = /^\s*(case|default)\b/.test(Q);
						return O.baseIndent + (e || a ? 0 : O.unit);
					},
					Block: S({ closing: "}" }),
					BlockComment: () => null,
					Statement: r({ except: /^{/ }),
...
function q() {
	let O = d.concat(_);
	return new n(b, [
		b.data.of({ autocomplete: o(U, c(O)) }),
		b.data.of({ autocomplete: G }),
	]);
}
export { q as go, b as goLanguage, G as localCompletionSource, d as snippets };
