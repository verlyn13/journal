...
async def handle_event(msg_data: dict[str, Any], s: AsyncSession):
    ...
    if etype in ("entry.created", "entry.updated"):
        eid = (
            msg_data.get("data", {}).get("id")
            or msg_data.get("aggregate_id")
            or msg_data.get("entry_id")
        )
        if not eid:
            return
...
async def main():
    ...
    # Durable consumers
    await js.subscribe(
        SUB_ENTRY, durable="embed-worker", cb=lambda m: asyncio.create_task(on_entry(nc, m))
    )
    await js.subscribe(
        SUB_REINDEX, durable="embed-reindexer", cb=lambda m: asyncio.create_task(on_reindex(nc, m))
    )
