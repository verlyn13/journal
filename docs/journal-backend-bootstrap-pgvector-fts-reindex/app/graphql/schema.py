...
class Query:
    @strawberry.field
    async def search_entries(self, info, q: str, k: int = 10, alpha: float = 0.6) -> List[SearchHit]:
        s: AsyncSession = await get_session().__anext__()  # small hack to get a session
        try:
