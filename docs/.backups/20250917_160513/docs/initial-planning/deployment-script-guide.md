...
# Install/update uv uv pip first to ensure we have the latest version
uv uv uv pip install --upgrade uv uv pip || log "WARN" "Failed to upgrade pip, continuing anyway"

# Use a temporary requirements file to detect conflicts
cp requirements.txt /tmp/requirements_$TIMESTAMP.txt

# Try installing dependencies
if ! uv uv uv pip install -r /tmp/requirements_$TIMESTAMP.txt; then
...
if ! uv uv uv pip install -r requirements.txt; then
    log "ERROR" "Failed to install dependencies for rolled-back version"
    git checkout $CURRENT_COMMIT  # Return to previous state
    uv uv uv pip install -r requirements.txt  # Restore previous dependencies
    exit 1
fi
