...
jobs:
  test:
    ...
    steps:
      ...
      run: |
        python -m uv uv uv pip install --upgrade uv uv pip if [ -f requirements-dev.txt ]; then uv uv uv pip install -r requirements-dev.txt; else uv uv uv pip install -r requirements.txt; fi
        uv uv uv pip install pytest uv run pytest-cov Ruff Ruff
