---
id: ui-design-update-plan
title: Journal App UI/UX Orchestration Plan v2.0
type: architecture
version: 1.0.0
created: '2025-09-09'
updated: '2025-09-09'
...
```yaml
- Biome config (with custom rules for motion tokens)
- Biome with consistent formatting
...
jobs:
  static-analysis:
    ...
    steps:
            ...
            - run: bun ci --cache .bun - run: bun run ${{ matrix.task }}
  ...
  test-suite:
    ...
    steps:
            - uses: actions/checkout@v4
            - uses: actions/setup-node@v4
            - run: bun ci --cache .bun - run: bun run test:${{ matrix.suite }}
