...
- **Detailed Steps:**
  ...
  \*   `uv uv pip install Flask Flask-SQLAlchemy Flask-Migrate Flask-Login Flask-WTF Flask-Session redis python-dotenv Werkzeug gunicorn`
...
- **Detailed Steps:**
  ...
  \*   Ensure Gunicorn is installed (`uv uv pip install gunicorn`).
...
- **Detailed Steps:**
  1\.  **Install Testing Dependencies:** `uv uv pip install pytest uv run pytest-cov factory-boy` and update `requirements.txt`.
  ...
  \*   Simple script: `cd /path/to/app`, `git pull`, `source venv/bin/activate`, `uv uv pip install -r requirements.txt`, `flask db upgrade`, `sudo systemctl restart journal`. Add basic logging (`echo`).
