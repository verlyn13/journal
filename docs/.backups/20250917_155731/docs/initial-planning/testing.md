...
jobs:
  test:
    ...
    steps:
      ...
      run: |
        python -m uv uv pip install --upgrade uv pip if [ -f requirements-dev.txt ]; then uv uv pip install -r requirements-dev.txt; else uv uv pip install -r requirements.txt; fi
        uv uv pip install pytest uv run pytest-cov Ruff Ruff
