---
id: user-management-orchestratev4
title: "M1.T3 \u2014 Refresh rotation, sessions, logout (flagged)"
type: api
version: 1.0.0
created: '2025-09-10'
updated: '2025-09-10'
...
### `apps/api/app/api/auth.py` (login → create session; refresh/logout)
...
pytestmark = uv run pytest.mark.integration

def _register_verify_login(api):
    os.environ["JOURNAL_USER_MGMT_ENABLED"] = "true"
    r = api.post("/api/auth/register", json={
        "email":"rot@journal.local","password":"CorrectHorse9!","username":"rot"
    })
    token = r.json().get("dev_verify_token")
    api.post("/api/auth/verify-email", json={"token": token})
    r = api.post("/api/auth/login", json={"email":"rot@journal.local","password":"CorrectHorse9!"})
...
def test_refresh_rotation_and_logout(api_client):
    ...
    r = api_client.post("/api/auth/refresh", json={"refresh_token": refresh1})
    assert r.status_code == 200
    refresh2 = r.json()["refresh_token"]
    assert refresh2 != refresh1
    r2 = api_client.post("/api/auth/refresh", json={"refresh_token": refresh1})
    assert r2.status_code == 401
    r3 = api_client.post("/api/auth/logout", json={"refresh_token": refresh2})
    assert r3.status_code == 204
    r4 = api_client.post("/api/auth/refresh", json={"refresh_token": refresh2})
