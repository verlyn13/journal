---
id: user-management-orchestratev3
title: "M1.T2 \u2014 Registration + Email Verify + Password Login (Argon2id)"
type: api
version: 1.0.0
created: '2025-09-10'
updated: '2025-09-10'
...
- New routes (404 if settings.user_mgmt_enabled is False):
  POST /api/auth/register -> 202 { message, dev_verify_token? }
...
### `apps/api/app/api/auth.py` (new endpoints under flag)
...
-from apps.api.app import get_session  # your app's session provider
+import jwt
...
+from apps.api.app.settings import settings
+from apps.api.app import get_session
...
def test_register_verify_login(api_client):
    ...
    # Verify email
    r = api_client.post("/api/auth/verify-email", json={"token": token})
