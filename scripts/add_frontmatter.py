...
class FrontmatterGenerator:
    ...
    def process_all(self, auto: bool = False, batch_size: int = 10):
        ...
        if not auto and files_to_process:
            response = input(
                f"Add frontmatter to {len(files_to_process)} files? (y/n): "
            )
            if response.lower() != "y":
                print("Aborted.")
                return
    ...
    def _generate_tags(self, file_path: Path, content: str, doc_type: str) -> List[str]:
        ...
        if "react" in content_lower:
            tags.append("react")
        if (
            "typescript" in content_lower
            or ".ts" in content_lower
            or ".tsx" in content_lower
        ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Add frontmatter to documentation files"
    )
    parser.add_argument(
        "--auto",
        action="store_true",
