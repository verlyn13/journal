...
def check_required_sections(body: str, sections: List[str]) -> List[str]:
    ...
    for sec in sections:
        ...
        pattern = re.compile(
            rf"^\s*#+\s*{re.escape(sec)}\s*$", re.IGNORECASE | re.MULTILINE
        )
...
def effective_sla_days(
    tax: Dict[str, Any], doc_type: str, priority: Optional[str]
) -> Optional[int]:
...
def main() -> int:
    ...
    for md in DOCS.rglob("*.md"):
        ...
        if doc_type and doc_type not in tax["types"]:
            ...
            issues.append(
                Issue(str(md.relative_to(DOCS)), f"Unknown type '{doc_type}'", "type")
            )
        else:
            t = tax["types"].get(doc_type)
        ...
        if status and status not in tax["statuses"]:
            ...
            issues.append(
                Issue(str(md.relative_to(DOCS)), f"Invalid status '{status}'", "status")
            )

        # Freshness SLA
        if (
            tax["rules"].get("enforce_freshness_sla", False)
            and doc_type in tax["types"]
        ):
            days = effective_sla_days(tax, doc_type, priority)
            if days is not None:
                last = parse_date(
                    str(fm.get("last_verified") or fm.get("updated") or "")
                )
                if last is None:
                    ...
                    issues.append(
                        Issue(
                            str(md.relative_to(DOCS)),
                            "Missing or invalid last_verified/updated date",
                            "freshness",
                        )
                    )
                else:
                    ...
                    if age > days:
                        ...
                        issues.append(
                            Issue(
                                str(md.relative_to(DOCS)),
                                f"Stale by {age - days} days (age {age}d > SLA {days}d)",
                                "freshness",
                            )
                        )
