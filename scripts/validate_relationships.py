...
def main() -> int:
        ...
        for doc_id, entry in docs.items():
            ...
            for r in refs:
                if r not in ids:
                    issues.append(
                        RelIssue(
                            "reference", f"{doc_id}: '{field}' target not found: {r}"
                        )
                    )
    ...
    for fld in ("parent",):
        ...
        for doc_id, entry in docs.items():
            ...
            if parent and parent not in ids:
                issues.append(
                    RelIssue("reference", f"{doc_id}: parent not found: {parent}")
                )
            ...
            for child in entry.get("children", []) or []:
                ...
                if child_entry.get("parent") != doc_id:
                    issues.append(
                        RelIssue(
                            "consistency",
                            f"Child '{child}' does not point back to parent '{doc_id}'",
                        )
                    )
    ...
    for cat, members in categories.items():
        for m in members:
            if m not in ids:
                issues.append(
                    RelIssue("category", f"Category '{cat}' references unknown id: {m}")
                )
    ...
    for sect, members in nav.items():
        for m in members:
            if m not in ids:
                issues.append(
                    RelIssue(
                        "navigation", f"Navigation '{sect}' references unknown id: {m}"
                    )
                )
