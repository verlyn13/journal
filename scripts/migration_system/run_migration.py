...
from scripts.migration_system.task_orchestrator import TaskOrchestrator, Task as TaskDefinition
...
class MigrationRunner:
    ...
    def define_migration_tasks(self) -> List[TaskDefinition]:
        ...
        tasks.append(TaskDefinition(
            ...
            priority=30,
            dependencies=["generate_index", "fix_internal_links", "check_freshness"],
            params={}
        ))
    ...
    def prepare_batch_tasks(self, tasks: List[TaskDefinition]) -> List[TaskDefinition]:
        ...
        for task in tasks:
            if task.id == "generate_frontmatter_all":
                ...
                scan_file = self.project_root / "docs" / "_generated" / "reports" / "frontmatter_scan.json"
                if scan_file.exists():
                ...
                quality_file = self.project_root / "docs" / "_generated" / "reports" / "quality_report.json"
    ...
    def run_migration(self, dry_run: bool = False, resume: bool = False) -> bool:
        ...
        try:
            ...
            while not self.orchestrator.all_tasks_done():
                stats = self.orchestrator.get_statistics()
                print(f"Progress: {stats['completed']}/{stats['total']} completed, {stats['failed']} failed")
                time.sleep(5)
    ...
    def validate_environment(self) -> bool:
        ...
        if not self.project_root.exists():
            checks.append(("Project root exists", False, f"Path not found: {self.project_root}"))
        else:
        ...
        if not docs_dir.exists():
            checks.append(("Docs directory exists", False, f"Path not found: {docs_dir}"))
        else:
        ...
        if not scripts_dir.exists():
            checks.append(("Scripts directory exists", False, f"Path not found: {scripts_dir}"))
        else:
