...
class DocumentationFixWorkflow:
    ...
    def __init__(self, docs_dir: Path = None, dry_run: bool = False):
        ...
        self.backup_dir = Path('docs/.backups') / datetime.now().strftime('%Y%m%d_%H%M%S')
        self.baseline = {}
        self.final_health = {}

    def execute_workflow(self, start_step: int = 1):
        """Execute the complete fix workflow."""
        print(f"🚀 Documentation Fix Workflow Starting...")
    ...
    def _add_frontmatter(self, file_path: Path):
        ...
        if self.dry_run:
            print(f"   [DRY RUN] Would add frontmatter to: {file_path.relative_to(self.docs_dir)}")
            return
        ...
        try:
            ...
            self.fixes_applied['frontmatter'].append(str(file_path.relative_to(self.docs_dir)))
        ...
        for md_file in self.docs_dir.rglob("*.md"):
            ...
            try:
                ...
                if content != original_content:
                    ...
                    self.fixes_applied['tools'].append(str(md_file.relative_to(self.docs_dir)))
                    files_updated += 1
                    print(f"   ✓ Updated tools in: {md_file.relative_to(self.docs_dir)}")
        ...
        for md_file in self.docs_dir.rglob("*.md"):
            ...
            try:
                ...
                def fix_link(match):
                    ...
                    # Skip external links
                    if any(link_target.startswith(p) for p in ['http://', 'https://', '#', 'mailto:']):
                    ...
                    if link_target.endswith('.md'):
                        if link_target not in all_files:
                            ...
                            fixed_link = self._find_correct_link(link_target, all_files, md_file)
                            if fixed_link:
                ...
                if content != original_content:
                    if not self.dry_run:
                        md_file.write_text(content)
                    self.fixes_applied['links'].append(str(md_file.relative_to(self.docs_dir)))
    ...
    def _find_correct_link(self, broken_link: str, all_files: Dict, current_file: Path) -> Optional[str]:
    ...
    def step5_clean_directories(self):
        ...
        for empty_dir in empty_dirs:
            if self.dry_run:
                print(f"   [DRY RUN] Would remove: {empty_dir.relative_to(self.docs_dir)}")
            else:
                try:
                    empty_dir.rmdir()
                    self.fixes_applied['directories'].append(str(empty_dir.relative_to(self.docs_dir)))
        ...
        for base_name, files in file_groups.items():
            if len(files) > 1:
                ...
                for f in files:
                    if f != best_file:
                        if self.dry_run:
                            print(f"   [DRY RUN] Would archive: {f.relative_to(self.docs_dir)}")
                        else:
                            self._archive_file(f)
                            duplicates_resolved += 1
                            duplicates_archived += 1
                            self.fixes_applied['duplicates'].append(str(f.relative_to(self.docs_dir)))
        ...
        for md_file in self.docs_dir.rglob("*.md"):
            ...
            try:
                ...
                if content != original_content:
                    if not self.dry_run:
                        md_file.write_text(content)
                    self.fixes_applied['quality'].append(str(md_file.relative_to(self.docs_dir)))
                    quality_fixes += 1
                    print(f"   ✓ Fixed quality in: {md_file.relative_to(self.docs_dir)}")
    ...
    def step8_update_outdated(self):
        ...
        for md_file in self.docs_dir.rglob("*.md"):
            if any(skip in str(md_file) for skip in ['_generated', 'archive', '.backups', 'migration']):
                continue
            ...
            try:
                ...
                if content != original_content:
                    if not self.dry_run:
                        md_file.write_text(content)
                    self.fixes_applied['outdated'].append(str(md_file.relative_to(self.docs_dir)))
                    outdated_fixed += 1
                    print(f"   ✓ Updated outdated content in: {md_file.relative_to(self.docs_dir)}")
    ...
    def step9_validate(self):
        ...
        try:
            ...
            if result.stdout:
                ...
                self.stats['final_issues'] = sum(len(v) for v in data.get('issues', {}).values())
    ...
    def step10_generate_report(self):
        ...
        unique_files = sorted({f for files in self.fixes_applied.values() for f in files})
        report.extend([
        ...
        report.extend([
            ...
            "",
            "---",
            f"*Report generated by documentation_fix_workflow.py*"
        ])
    ...
    def _capture_quick_health(self) -> Dict:
        ...
        metrics = {}
        try:
            result = subprocess.run(['bash', 'scripts/check_docs.sh'], capture_output=True, text=True)
            out = result.stdout
            # Parse key values
    ...
    def _capture_baseline(self):
        ...
        if self.baseline.get('frontmatter_percent') is not None:
            self.stats['baseline_frontmatter_percent'] = self.baseline['frontmatter_percent']
...
Examples:
  ...
  python scripts/documentation_fix_workflow.py --step 4  # Start from step 4
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Start from specific step (1-10)'
    )
