...
class DocumentationCoverageChecker:
    ...
    def _check_api_coverage(self):
        ...
        self.coverage_data["api_coverage"] = {
            ...
            "percentage": len(documented_endpoints) / (len(documented_endpoints) + len(undocumented_endpoints)) * 100
            if documented_endpoints or undocumented_endpoints else 0
        }
    ...
    def _check_component_coverage(self):
        ...
        self.coverage_data["component_coverage"] = {
            ...
            "percentage": len(documented_components) / (len(documented_components) + len(undocumented_components)) * 100
            if documented_components or undocumented_components else 0
        }
                ...
                if len(parts) >= 3:
                    try:
                        ...
                        if 'updated' in frontmatter:
                            updated_date = datetime.strptime(frontmatter['updated'], '%Y-%m-%d')
    ...
    def _calculate_summary(self):
        """Calculate summary metrics."""
        total_docs = sum(len(files) for files in self.docs_dir.rglob("*.md")
                        if "_generated" not in str(files))

        incomplete_count = sum(len(items) for items in self.coverage_data["incomplete"].values())
        outdated_count = sum(len(items) for items in self.coverage_data["outdated"].values())
        missing_count = sum(len(items) for items in self.coverage_data["missing"].values())
        ...
        self.coverage_data["summary"] = {
            ...
            "api_coverage": self.coverage_data.get("api_coverage", {}).get("percentage", 0),
            "component_coverage": self.coverage_data.get("component_coverage", {}).get("percentage", 0),
            "overall_health": max(0, 100 - (incomplete_count + outdated_count + missing_count) * 2)
        }
...
def print_coverage_report(coverage_data: Dict[str, Any]):
    ...
    if coverage_data.get("component_coverage", {}).get("undocumented")[:5]:  # Show first 5
        ...
        for doc_type, files in coverage_data["outdated"].items():
            if files:
                ...
                for file_info in files[:2]:  # Show first 2 per type
                    print(f"    - {file_info['file']} ({file_info['days_old']} days old)")
