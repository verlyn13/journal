...
class DocumentValidator:
        ...
        for line in lines:
            ...
            if match:
                ...
                if current_section:
                    ...
                    current_section["token_count"] = self._estimate_tokens(
                        current_section["content"]
                    )
    ...
    def _extract_relationships(self, content: str) -> Dict[str, List[str]]:
        ...
        relationships = {"related": [], "prerequisites": [], "references": []}
    ...
    def _check_policies(
        ...
        for section in sections:
            ...
            if token_count < 150:
                warnings.append(
                    f"Section too short ({token_count} tokens): {section['heading']}"
                )
            elif token_count > 1000:
                warnings.append(
                    f"Section too long ({token_count} tokens): {section['heading']}"
                )
